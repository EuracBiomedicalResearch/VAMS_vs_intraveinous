...
```{r heatmap capillary, fig.path = IMAGE_PATH, fig.width = 15, fig.height = 10, fig.cap = "Heatmap of features for capillary samples.", echo = FALSE}
## Create heatmap
tmp <- log2(assay(
...
tab <- res_table(res_ven)
```

Next, we generate a heatmap containing only the signals from the features
identified above to identify any similarities among samples.

```{r heatmap VBS, fig.path = IMAGE_PATH, fig.width = 15, fig.height = 10, fig.cap = "Heatmap of features for venous.", echo = FALSE}
## Create heatmap
tmp <- log2(assay(
...
tab <- res_table(res_plas)
```

Next, we generate a heatmap containing only the signals from the features
identified above to identify any similarities among samples.

```{r heatmap plasma, fig.path = IMAGE_PATH, fig.width = 15, fig.height = 10, fig.cap = "Heatmap of features for venous samples.", echo = FALSE}
## Create heatmap
tmp <- log2(assay(
...
Of the top 100 features with lowest p-value, 31 were found common between capillary and 
venous blood samples, 14 between capillary and plasma, 8 between venous and
plasma and 7 in all samples.
...
ids <- unique(c(rownames(top_100_cap), rownames(top_100_ven),
                rownames(top_100_plas)))
...
Overlap between capillary and venous is higher. For a number of features, the
sign of the coefficient in plasma samples differs to the sign in capillary and
venous samples.

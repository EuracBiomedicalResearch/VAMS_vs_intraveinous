...
# Differential abundance analysis: **capillary* blood samples
...
```{r peak-shape, echo = FALSE, eval = FALSE}
...
TODO: we want to know how similar the results found in the 3 sample matrices
are. We're less interested in actually identifying the metabolites that are
different between male/female, but we want to know if we would get similar
results when analyzing whole blood (venous), plasma or capillary samples.

- compare the list of significant features. (using the `UpSetR` package).
- compare the list of top 100 features with smallest p-value.
- directly plot (compare) coefficients? Plot coefficients of all features from
  venous against coefficients of all features in capillary and the same between
  venous and plasma and capillary and plasma.
- potential problem: can not expect the same features in all 3 matrices - maybe
  reduce to metabolites present in ~ 50% of samples for **each** matrix and
  repeat the diff abundance analysis?
- Use different ways to pre-filter the data? How does this affect the results
  (number of significant features) and the overlap of significant features?
  - rsds < 0.3
  - dratio < 0.5 in all matrices
...
We start with the analysis of the **capillary** blood samples:

```{r sample-ranks, capillary blood samples}
...
```{r sample-ranks-average, capillary blood samples}
...
```{r export-result capillarz blood samples, echo = FALSE}
...
# Differential abundance analysis: **venous** blood samples

```{r sample-ranks ven}
abundance_mat <- assay(res_ven, "normalized_filled_imputed")[rownames(tab), ]
sample_ranks <- t(apply(-abundance_mat, MARGIN = 1, rank))
```
...
```{r sample-ranks-cap-table ven, echo = FALSE, results = "asis"}
F_table <- abundance_mat[rownames(sample_ranks_F),
                           order(sample_ranks_F_sum)]
F_table <- F_table[, 1:6]
pandoc.table(
...
```{r sample-ranks-M-table ven, echo = FALSE, results = "asis"}
M_table <- abundance_mat[rownames(sample_ranks_M),
                           order(sample_ranks_M_sum)]
M_table <- M_table[, 1:6]
pandoc.table(
...
```{r initial-feature-groups ven}
data_study <- filterFile(data_pos, match(res_ven$mzML_file, data_pos$mzML_file),
                         keepFeatures = TRUE)
...
fgs <- rep(NA_character_, nrow(featureDefinitions(data_study)))
names(fgs) <- rownames(featureDefinitions(data_study))
fgs[rownames(rowData(res_ven))[rowData(res_ven)$significant]] <- "FG"
featureGroups(data_study) <- fgs
...
```{r feature-grouping-retention-time ven, fig.path = IMAGE_PATH, fig.cap = "Feature grouping result based on similar retention time.", echo = FALSE, fig.width = 5 * phi, fig.height = 5}
plotFeatureGroups(data_study)
grid()
```
...
```{r VBS, echo = FALSE, results = "asis"}
fgroups <- featureGroups(data_study)
names(fgroups) <- rownames(featureDefinitions(data_study))
fgroups <- fgroups[!is.na(fgroups)]
fgroups <- sort(fgroups)

tab <- rowData(res_ven)[names(fgroups), c("mzmed", "rtmed", "coef", "adjp")]
tab <- cbind(as.data.frame(tab), fgroup = fgroups)
...
```{r feature-grouping-cormat ven, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of feature abundances across samples.", echo = FALSE, fig.height = 10, fig.width = 10}
fvals <- log2(featureValues(
    data_study, filled = TRUE, method = "sum")[rownames(tab), ])
...
```{r venous, echo = FALSE, results = "asis"}
fgroups <- featureGroups(data_study)
names(fgroups) <- rownames(featureDefinitions(data_study))
fgroups <- fgroups[!is.na(fgroups)]
fgroups <- sort(fgroups)
...
```{r eic-correlation ven, message = FALSE, warning = FALSE}
data_study <- groupFeatures(
    data_study, EicCorrelationParam(threshold = 0.7, n = 10, clean = TRUE))
```
...
At last, we create overview plots for all feature groups (code hidden).

```{r feature-grouping-overview-plots ven, echo = FALSE}
fgs <- unique(featureGroups(data_study))
fgs <- fgs[!is.na(fgs)]

dr <- paste0(IMAGE_PATH, "feature_groups_ven/")
dir.create(dr, showWarnings = FALSE)
for (fg in fgs) {
    fts_idx <- which(featureGroups(data_study) == fg)
    eics <- featureChromatograms(data_study, features = fts_idx,
...
rowData(res_ven)$feature_group <- fgs[rownames(rowData(res_ven))]
save(res_ven, file = paste0(RDATA_PATH, "res_ven.RData"))

tmp <- rowData(res_ven)
tmp <- cbind(feature = rownames(tmp), tmp)
write_xlsx(as.data.frame(tmp),
           path = paste0(RESULT_PATH, FILE_NAME, "-res_ven.xlsx"))
```

# Differential abudance analysis: **plasma** samples

```{r sample-ranks plasma}
abundance_mat <- assay(res_plas, "normalized_filled_imputed")[rownames(tab), ]
sample_ranks <- t(apply(-abundance_mat, MARGIN = 1, rank))
```
...
```{r sample-ranks-cap-table plasma, echo = FALSE, results = "asis"}
F_table <- abundance_mat[rownames(sample_ranks_F),
                           order(sample_ranks_F_sum)]
F_table <- F_table[, 1:6]
pandoc.table(
...
```{r sample-ranks-M-table plasma, echo = FALSE, results = "asis"}
M_table <- abundance_mat[rownames(sample_ranks_M),
                           order(sample_ranks_M_sum)]
M_table <- M_table[, 1:6]
pandoc.table(
...
```{r initial-feature-groups plasma}
data_study <- filterFile(data_pos, match(res_plas$mzML_file, data_pos$mzML_file),
                         keepFeatures = TRUE)
...
fgs <- rep(NA_character_, nrow(featureDefinitions(data_study)))
names(fgs) <- rownames(featureDefinitions(data_study))
fgs[rownames(rowData(res_cap))[rowData(res_plas)$significant]] <- "FG"
featureGroups(data_study) <- fgs
...
```{r feature-grouping-retention-time plssma, fig.path = IMAGE_PATH, fig.cap = "Feature grouping result based on similar retention time.", echo = FALSE, fig.width = 5 * phi, fig.height = 5}
plotFeatureGroups(data_study)
grid()
```
...
```{r plasma, echo = FALSE, results = "asis"}
fgroups <- featureGroups(data_study)
names(fgroups) <- rownames(featureDefinitions(data_study))
fgroups <- fgroups[!is.na(fgroups)]
fgroups <- sort(fgroups)

tab <- rowData(res_plas)[names(fgroups), c("mzmed", "rtmed", "coef", "adjp")]
tab <- cbind(as.data.frame(tab), fgroup = fgroups)
...
```{r feature-grouping-cormat plas, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of feature abundances across samples.", echo = FALSE, fig.height = 10, fig.width = 10}
fvals <- log2(featureValues(
    data_study, filled = TRUE, method = "sum")[rownames(tab), ])
...
```{r eic-correlation plas, message = FALSE, warning = FALSE}
data_study <- groupFeatures(
    data_study, EicCorrelationParam(threshold = 0.7, n = 10, clean = TRUE))
```
...
We create overview plots for all feature groups (code hidden).

```{r feature-grouping-overview-plots plasma, echo = FALSE}
fgs <- unique(featureGroups(data_study))
fgs <- fgs[!is.na(fgs)]

dr <- paste0(IMAGE_PATH, "feature_groups_plas/")
dir.create(dr, showWarnings = FALSE)
for (fg in fgs) {
    fts_idx <- which(featureGroups(data_study) == fg)
    eics <- featureChromatograms(data_study, features = fts_idx,
...
```{r export-result plas, echo = FALSE}
fgs <- featureGroups(data_study)
names(fgs) <- rownames(featureDefinitions(data_study))

rowData(res_plas)$feature_group <- fgs[rownames(rowData(res_plas))]
save(res_cap, file = paste0(RDATA_PATH, "res_plas.RData"))

tmp <- rowData(res_plas)
tmp <- cbind(feature = rownames(tmp), tmp)
write_xlsx(as.data.frame(tmp),
           path = paste0(RESULT_PATH, FILE_NAME, "-res_plas.xlsx"))
```

...
```
```{r}
pdf(file = paste0(IMAGE_PATH, "chrs_bpc.pdf"), height = 5, width = 5*phi)
  plot(chrs_bpc, col = paste0(col_source[as.character(chrs_bpc$source)], 60))
  abline(v = rt_cut)
  legend("topright", col = col_source, legend = names(col_source), lwd = 2)
dev.off()
```
...
```
```{r}
pdf(file = paste0(IMAGE_PATH, "bpc2_cluster.pdf"), height = 20, width = 10*phi)
cormat <- cor(log2(do.call(cbind, lapply(chrs_bin, intensity))))
colnames(cormat) <- rownames(cormat) <- sampleNames(data_pos)
ann <- pData(data_pos)[, c("batch", "source")]
ann$batch <- factor(ann$batch, levels = c("31012018", "01022018"))
...
and capillary blood samples. For individuals *16* and *19* both replicates are affected suggesting a potential problem with the extraction or the sample tube. 
All the others, namely samples *11*, *12*, *20* and *22*, the actual measurement might be failed, since the other replicate was not affected. Note that all 
...
```
```{r}
pdf(file = paste0(IMAGE_PATH, "boxplots.pdf"), height = 5, width = 10*phi)
boxplot(lapply(bpcs, log2), col = paste0(col_source[chrs_bpc$source], 60),
        pch = NA, xaxt = "n", ylab = expression(log[2]~base~peak~intensity),
        xlim = xl)
plot(bpcs_rla, xlab = "", ylab = "average RLA", xlim = xl,
...
pc <- prcomp(t(log2(do.call(cbind, lapply(chrs_bin, intensity)))),
             scale = FALSE, center = TRUE)
par(mfrow = c(2, 2))
plot_pca(pc, col = col_source[as.character(data_pos$source)], pc_x = 1, pc_y = 2)
legend("topleft", col = col_source, legend = names(col_source), lwd = 2, cex=0.4)
plot_pca(pc, col = col_source[as.character(data_pos$source)], pc_x = 3, pc_y = 4)
legend("topleft", col = col_source, legend = names(col_source), lwd = 2, cex=0.4)
plot_pca(pc, col = col_source[as.character(data_pos$source)], pc_x = 2, pc_y = 3)
legend("topleft", col = col_source, legend = names(col_source), lwd = 2, cex=0.4)
```
```{r}
pdf(file = paste0(IMAGE_PATH, "pca.pdf"), height = 7, width = 5*phi)
plot_pca(pc, col = col_source[as.character(data_pos$source)], pc_x = 1, pc_y = 2)
legend("topleft", col = col_source, legend = names(col_source), lwd = 2, cex=0.4)
plot_pca(pc, col = col_source[as.character(data_pos$source)], pc_x = 3, pc_y = 4)
legend("topleft", col = col_source, legend = names(col_source), lwd = 2, cex=0.4)
...
dr <- paste0(IMAGE_PATH, "peakdetection_3/")
...
pdf(file = paste0(IMAGE_PATH, "glucose_bevor.pdf"), height = 7, width = 5)
pdp_tmp <- PeakDensityParam(sampleGroups = data_pos$source, bw = 1.3)
plot_grouping(data_pos, mz = mzr, rt = rtr, param = pdp_tmp,
              col = col_source[data_pos$source])
```
...
plot_grouping(data_pos, mz = mzr, rt = rtr, param = pdp_tmp,
              col = col_source[data_pos$source])
pdf(file = paste0(IMAGE_PATH, "glucose_bw_3.pdf"), height = 7, width = 5)
pdp_tmp <- PeakDensityParam(sampleGroups = data_pos$source, bw = 3)
plot_grouping(data_pos, mz = mzr, rt = rtr, param = pdp_tmp,
              col = col_source[data_pos$source])
```
...
pdf(file = paste0(IMAGE_PATH, "alignment_gg.pdf"), height = 7, width = 9)
par(mfrow = c(3, 1), mar = c(0, 4.3, 2, 0.1))
plot(chr_raw, col = paste0(col_source[chr_raw$source], 40),
     peakType = "none", main = "BPC, raw")
plot(chr_adj, col = paste0(col_source[chr_raw$source], 40),
...
```{r pos-alignment-kc-plots, echo = FALSE, warning = FALSE, message = FALSE, eval = plot_eic}
...
or 3-Methylhistidine below, others could not be aligned perfectly, such
as C4 Carnitine (see last image below).

![](images/vams_preprocessing_pos/alignment/48-serine_POS.png)

![](images/vams_preprocessing_pos/alignment/49-3-methylhistidine_POS.png)

![](images/vams_preprocessing_pos/alignment/5-c4_POS.png)

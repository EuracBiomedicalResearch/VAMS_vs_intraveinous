...
MZML_PATH <- "/Users/jo/data/mzML/2018/"
...
the peaks for Leucine and Isoleucine (data not shown). Below we perform the peak
grouping on the defined settings and subsequently the alignment. For the
alignment we require a *hook* peak to be present in at least 80% of samples. We
also increase the smoothing parameter for the loess function (`span =
0.3`). With the default value of `span = 0.2` alignment resulted in spectra
within some files to be not ordered by retention time anymore. Setting the
parameter `extraPeaks = 50` ensured *hook* peaks not being discarded because of
the grouping of additional peaks within the same sample to the same
feature. Setting this parameter to a large value resulted in *hook* peaks that
span almost the full retention time range.
...
for (i in seq_len(nrow(std_info))) {
    ...
    cmpname <- tolower(gsub(" .*", "", cmpname))
    filename <- paste0(dr, i, "-", cmpname, "_", "POS", ".png")
    rtr <- std_info[i, "RT_obs"] + c(-7, 7)
    mzr <- std_info[i, "mz_ion"]
    pks <- chromPeaks(data_pos, rt = rtr, mz = mzr, type = "apex_within",
                      ppm = 50)
    if (!nrow(pks)) {
        #' try again with a larger ppm...
        pks <- chromPeaks(data_pos, rt = rtr, mz = mzr, type = "apex_within",
                          ppm = 100)
    }
    if (nrow(pks)) {
        rtr <- range(pks[, c("rtmin", "rtmax")], rtr) + c(-2, 2)
        mz_pks <- range(pks[, c("mzmin", "mzmax")])
    } else mz_pks <- mzr + c(-0.01, 0.01)
    chr_raw <- chromatogram(data_raw, rt = rtr, mz = mz_pks)
    chr_adj <- chromatogram(data_pos, rt = rtr, mz = mz_pks)
    png(filename, width = 16, height = 8, units = "cm",
        res = 200, pointsize = 4)
...
![](images/vams_preprocessing/alignment/43-serine_POS.png)

![](images/vams_preprocessing/alignment/73-1-methylhistidine_POS.png)

![](images/vams_preprocessing/alignment/41-taurine_POS.png)
...
for (i in seq_len(nrow(std_info))) {
    ...
    chr <- chromatogram(data_pos, rt = rtr, mz = mz_pks)
    ...
    png(filename, width = 12, height = 10, units = "cm",
        res = 200, pointsize = 4)
    plotChromPeakDensity(chr, simulate = FALSE, main=std_info_good$name[i],
                         peakCol = paste0(pk_col, 60),

...
for (i in 1:nrow(pos_cmps)) {
    ...
    rtr[2] <- rtr[2] + 2
    mz_pks <- range(pks[, c("mzmin", "mzmax")])
    fn <- paste0(IMAGE_PATH, pos_cmps$id[i], "_chrom_plot_correspondence.png")
    png(fn, width = 10, height = 8, units = "cm", res = 200, pointsize = 6)
    plot_grouping(data_pos, rt = rtr, mz = mz_pks, param = pdp,
                  col = paste0(col_source[data_pos$source], 80),
                  simulate = FALSE)
    dev.off()
}
...
for (feat in feats_low) {
    ...
    png(fn, width = 10, height = 8, units = "cm", res = 200, pointsize = 6)
    plot_feature(data_pos, feat, col = paste0(col_source[data_pos$source], 40),
...
ftr <- "FT01973"
ft <- featureDefinitions(data_pos)[ftr, ]
pks <- chromPeaks(data_pos)[unlist(ft$peakidx), ]
mzr <- range(pks[, c("mzmin", "mzmax")])
rtr <- range(pks[, c("rtmin", "rtmax")])
...
data_lrg <- filterFile(data_pos, pks_lrg[, "sample"])
...
ft <- featureDefinitions(data_pos)["FT06230", ]
pks <- chromPeaks(data_pos)[unlist(ft$peakidx), ]
mzr <- range(pks[, c("mzmin", "mzmax")])
rtr <- range(pks[, c("rtmin", "rtmax")])
rtr <- c(rtr[1] - 10, rtr[2] + 10)
## In which samples do we have multipeaks?
tbl <- table(chromPeaks(data_pos)[unlist(ft$peakidx), "sample"]
smps <- as.numeric(names(tbl)[tbl > 1])

par(mfrow = c(1, 2), mar = c(4, 4.5, 1.5, 0.5))
plot(chromatogram(data_pos, mz = mzr, rt = rtr),
     col = paste0(col_source[data_pos$source], 60))
highlightChromPeaks(data_pos, rt = rtr, mz = mzr, whichPeaks = "apex_within",
                    border = paste0(col_source[data_mlt$source], 20))
data_mlt <- filterFile(data_pos, smps)
...
## Get the data...
ft <- featureDefinitions(data_pos)["FT03031", ]
pks <- chromPeaks(data_pos)[unlist(ft$peakidx), ]
mzr <- range(pks[, c("mzmin", "mzmax")])
rtr <- range(pks[, c("rtmin", "rtmax")])
...
tmp <- filterMz(filterRt(filterFile(data_pos, smps), rt = rtr), mz = mzr)
...
```{r multi-single-define, echo = FALSE, results = "asis"}
...
```{r multi-single-plot, echo = FALSE, results = "silent"}
...
for (feat in rownames(tab)) {
    fn <- paste0(dr, feat, ".png")
    png(fn, width = 10, height = 8, units = "cm", res = 200, pointsize = 6)
    plot_feature(data_pos, feat, col = paste0(col_source[data_pos$source], 60),
...
```{r overlapping-features, message = FALSE}
...
```{r feature-groups-close-rt, message = FALSE, echo = FALSE}
...
for (i in seq_along(ft_grps)) {
    ...
    plot_feature(data_pos, ft_grps[[i]],
                 col = paste0(col_source[data_pos$source], 40),
                 peak_type = "rect", lwd = 2, simulate = FALSE)
    dev.off()
}
...
```{r feature-groups-close-ppm, echo = FALSE, results = "hide"}
...
for (i in seq_along(fts_grps)) {
    ...
    ## Use the pool data...
    data_qc <- filterFile(data_pos, file = which(data_pos$source == "all"))
...
save(data_pos, paste0(RDATA_PATH, "data_pos.RData"))

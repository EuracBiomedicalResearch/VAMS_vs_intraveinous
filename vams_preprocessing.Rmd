...
We next cluster samples based on their base peak chromatograms.

```{r pos-bin-chr, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 12, fig.height = 12, fig.cap = "Samples clustered by base peak chromatogram. Clustering bases on pairwise correlations of base peak chromatograms."}
## First we have to bin the chromatograms to force them having the same
## retention times.
chrs_bin <- bin(chrs_bpc)

cormat <- cor(do.call(cbind, lapply(chrs_bin, intensity)))
...
capillary samples and red blood cells from all other sources (i.e. plasma, all,
and venous samples). One cluster of samples, consisting exclusively of capillary
and red blood cell samples, mostly from the second batch, separates from all
others samples.
...
- plot some random ones for
  - multi peaks with less than 10%.
  - multi peaks with > 50%.


- Correspondence summary:
  - total number of features.
  - total number of single-peak/sample features
  - number of features present in > 50% of samples per group (for each group).
  - number of single-peak/sample features present in > 50%...
...
peak assigned in many samples. 

### Features with different number of peaks depending on the source

- Identify features that have > 90% samples in at least one group with a single
  peak and > 90% of samples from another with multiple.

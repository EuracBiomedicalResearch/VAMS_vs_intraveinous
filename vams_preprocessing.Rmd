...
time_stamp <- extractTimeStamps(data_pos)
data_pos$time_stamp <- time_stamp
stopifnot(all(data_pos$inj_idx, order(data_pos$time_stamp)))
```
...
for (i in 1:nrow(pos_cmps)) {
    ...
    mz_exp <- pos_cmps$mz[i]
    mz_ppm <- mz_exp * 20 / 1e6         # for a ppm = 40
    mzr <- mz_exp + c(-mz_ppm, mz_ppm)
    rtr <- pos_cmps$rt_expected[i] + c(-20, 20)
    tmp_chr <- chromatogram(qc_pos, rt = rtr + c(-5, 5), mz = mzr)
    png(fn, width = 8, height = 6, units = "cm", res = 200, pointsize = 6)
    plot(tmp_chr, main = paste0(pos_cmps[i, "name"], ": ",
                                paste(format(mzr, digits = 6), collapse = "-")))
    abline(v = pos_cmps$rt_expected[i], lty = 3)
    dev.off()
}

#' mzML backend: took 505 sec
#' hdf5 backend: took 348 sec
#' hdf5 backens, serial processing:
...
kcs <- rbind(
    ...
    kc_36 = c(184, 192),           # 8000, two peaks, picked second.
    kc_43 = c(176, 182),           # 70000
    kc_44 = c(191, 198),           # 20000, mis-aligned
    kc_46 = c(170, 176),           # 40000
    kc_48 = c(155, 160),           # 3000, two peaks, picked second.
    kc_52 = c(191, 198),           # 30000, slightly mis-aligned
    kc_54 = c(26, 29),             # 1000
    kc_56 = c(165, 169),           # 2000
    kc_61 = c(153, 157),           # 12000, slightly mis-aligned. Could also be one of several peaks
    ...
    kc_79 = c(163, 170),           # 400
    kc_81 = c(155, 170),           # 6000, mis-aligned, 3 peaks (kc_81-83)
    kc_82 = c(155, 170),           # 6000, mis-aligned, 3 peaks (kc_81-83)
    kc_83 = c(155, 170),           # 6000, mis-aligned, 3 peaks (kc_81-83)
    kc_85 = c(159, 166),           # 8000, mis-aligned
    kc_86 = c(145, 149),           # 2500, mis-aligned (between batch?)
    kc_88 = c(159, 165),           # 800, mis-aligned, = kc_89
##    kc_90 = c(190, 200),           # 2000, mis-aligned
    ...
    kc_115 = c(30, 36),            # 4000 batch specific?
    kc_116 = c(184, 192),          # 50000
    kc_122 = c(28, 31),            # 120000 batch specific?
    kc_123 = c(165, 170)          # 1500
...
chromatographic peaks of the known compounds is for some compounds surprisingly
large. We use thus a`ppm = 50` to ensure that the *regions of interest* (ROIs) 
in which centWave performs the peak detection include the full, or the largest 
part, of the chromatographic peak of the ions. The average width of 
chromatographic peaks is about 7 seconds, thus we use a `peakwidth = c(2, 14)`.

```{r pos-centWave, message = FALSE, warning = FALSE, eval = !cached_peak_detection}
...
```{r pos-centWave-cached, echo = FALSE, eval = cached_peak_detection}
...
for (i in 1:nrow(pos_cmps)) {
    ...
    if (length(multi_idx)) {
        ...
        col_sub <- col_source[data_pos$source[chromPeaks(chr_sub)[, "sample"]]]

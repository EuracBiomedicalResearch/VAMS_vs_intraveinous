...
load("data/RData/vams_normalization/grpDta_pos.RData")
...
res_matrix_pos <- rowData(grpDta_pos)
Data <- log2(assays(grpDta_pos)$Imputed)[, ]

#A for loop in which each of the matrices are are passed to the above function
for (src in TestedSourcenames) {

  fitMatrix <- t(apply(Data[, which(grpDta_pos$source == src)], 1, fitlmSexFunction, 
    dataMat = colData(grpDta_pos)[which(grpDta_pos$source == src), ], model = conc ~ sex))
...
res_matrix_pos_Conf <- rowData(grpDta_pos)
Data <- log2(assays(grpDta_pos)$Imputed)[, ]

#As before #A for loop in which each of the matrices are are passed to the above function
for (src in TestedSourcenames) {
fitMatrix <- t(apply(Data[, which(grpDta_pos$source == src)], 1, fitlmSexFunction, 
  dataMat = colData(grpDta_pos)[which(grpDta_pos$source == src), ], model = conc ~ sex + age))
...
The others are mostly the same whereas the capillary changes from about 195 to
about 280, a more than a factor 2 difference
...
#' The rowdata of vamspos is put into the result matrix. Likewise the imputed data
#' is redefined into the container Data
res_matrix_pos_All <- rowData(grpDta_pos)
Data <- log2(assays(grpDta_pos)$Imputed)[, ]

fitMatrix <- t(apply(Data[, ], 1, fitlmSexFunction, 
  dataMat = colData(grpDta_pos)[, ], model = conc ~ sex + source))
...
above 2. This is in contrast to the individual models, where many features with
...
1300, and a very steep decrease. This indicates that the effect of confounding 
...
546 features were found to have unadjusted p-values below 0.01. This is more 

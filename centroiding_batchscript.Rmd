...
Note this relies on SQLite to keep track of what has been worked on. This is the
feature that also allows multiple instances to run concurrently on the computer.

```{r  whole script, message = FALSE, eval=FALSE}
#!/home/ssmarason/R/devel/bin/Rscript
...
for(i in 1:length(MZML_POLARITY)) {
    ...
    fpath <- paste0(MZML_PATH, MZML_IN,'mzML', onePol,"/")
    print(fpath)
    fls[[i]] <- list.files(fpath,'*.mzML')
}
    ...
    for(j in 1:length(fls[[i]])) {
        ...
        if(!check_done(SQLITE_PATH, mytable, onePol, oneFls)) {
            ...
            data_profile_none <- filterRt(
                        readMSData(
                            paste0(MZML_PATH, MZML_IN,'mzML', onePol,"/",oneFls),

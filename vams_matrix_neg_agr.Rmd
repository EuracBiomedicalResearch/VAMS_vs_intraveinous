...
#' find path
load(paste0(RDATA_PATH,"ExportTableNeg.RData"))

```

## Aggregating data Based on Groups from CAMERA Package
In this analysis the data from vams_neg is not used directly. As an attempt to
simplify the data the possible adducts were found using the CAMERA package. This
was done in [CAMERA package.Rmd](CAMERA package.Rmd). This was done on the
original xcms object data_pos. In the
[vams_normalization.Rmd](vams_normalization.Rmd) some features were found to be
too scarce to be included and these will have to be removed from the grouped
data.

```{r Subsetting-of-ExportTable}

presentFeatures <- rownames(ExportTable)
CAMERAData <- ExportTable[rownames(vams_neg),]
...
aggregationFunction <- function(Pattern, y, FUN = colSums) {
  ...
  Output$Abundance <-
  FUN(assay(vams_neg, "abundances")[logicalPattern,]
  , na.rm = TRUE)
  Output$Weight <-
  colMaxs(assay(vams_neg, "weights")[logicalPattern,]
  , na.rm = TRUE)
  Output$maxRow <- which.max(assay(vams_neg, "abundances")
  [logicalPattern,])
  
  }
  else {
  Output$Abundance <- assay(vams_neg, "abundances")[logicalPattern, ]
  Output$Weight <- assay(vams_neg, "weights")[logicalPattern, ]
...
Note that due to the way sums treat dropped columns with missing values, 0's are
...
groupedData <-
rbind(Grouped$Abundances, assay(vams_neg, "abundances")
[is.na(CAMERAData$adductGroup), ])
groupedDataWeights <-
rbind(Grouped$Weight, assay(vams_neg, "weights")
[is.na(CAMERAData$adductGroup), ])
...
rowDataGroup <-
rbind(rowData(vams_neg)[as.vector(Grouped$maxRow), ], rowData(vams_neg)
[is.na(CAMERAData$adductGroup), ])
...
save(grpDta_neg,
file = paste0("data/RData/vams_normalization/", "grpDta_neg.RData"))
...
tmp <- log2(assay(grpDta_neg, "Imputed"))
pc <- prcomp(t(tmp), scale = FALSE, center = TRUE)
pchs <- ifelse(grpDta_neg$sex == "F", 21, 22)

par(mfrow = c(1, 2), mar = c(4.5, 4.5, 1, 1))
plot_pca(pc, pc_x = 1, pc_y = 2, col = paste0(col_source[grpDta_neg$source], "ce"),
         bg = paste0(col_source[grpDta_neg$source], 60), pch = pchs, cex = 1.5)
legend("top", col = col_source, legend = names(col_source), pch = 16)
plot_pca(pc, pc_x = 3, pc_y = 4, col = paste0(col_source[grpDta_neg$source], 80),
         bg = paste0(col_source[grpDta_neg$source], 40), pch = pchs, cex = 1.5)

```

The PCA above was performed on all features . Missing values were replaced with
half of the feature's smallest abundance adding also a small random value based
on the features' standard deviations. These PCA-plots are mostly identical to
the ones using the entire dataset, As ony 800 features were removed this is not
very surprising. What is more surprising is that only using the aggregated data
(about 5000 features) gives the same results (Data not shown).
...
prop_detected <- apply(assay(grpDta_neg, "weights"), 1, function(x, grps) {
...
Note that the present analysis is based on features with a detected peak in at
...
```
Not unexpectedly, since most features are retained the distribtions are mostly
identical to the previous analysis. Most features are present in venous samples
followed by plasma and then capillary. The fewest are present in RBC samples
...
ints <- list(
    common = rowMeans(assay(grpDta_neg)[feats_common, ]),
    plasma = rowMeans(assay(grpDta_neg)[feats_plasma,
                                      grpDta_neg$source == "plasma"]),
    venous = rowMeans(assay(grpDta_neg)[feats_venous,
                                      grpDta_neg$source == "venous"]),
    RBC = rowMeans(assay(grpDta_neg)[feats_RBC,
                                   grpDta_neg$source == "RBC"]),
    capillary = rowMeans(assay(grpDta_neg)[feats_capillary,
                                         grpDta_neg$source == "capillary"])
)
par(mar = c(7, 4.5, 1, 1))
...
```{r Model-for-linear-model-VS-capillary, warning = FALSE}
...
Data <- log2(imputeRowMinRand(assay(grpDta_neg), min_fraction = 1/2,
                    sd_fraction = 1, abs = TRUE))
fitMatrix <- t(apply(Data, 1, fitlmFunction, 
                     dataMat = as.data.frame(colData(grpDta_neg)),
...
res_matrix_pos<- cbind(rowData(grpDta_neg), fitMatrix, adj = p.adjusted)
```

Histograms of each of the 3 matrixes against capillary is made.

```{r Histogram-of-p-values, fig.path = IMAGE_PATH, echo = FALSE, fig.cap = "Histogram of the p values each of the linear models against capillary. All histograms show a smooth decrease over the range of pvalues, indicating only a small possible effect from confounding factors"}
...
As expected, The reulsts match what was seen in the previous analysis the
...
plot(density(res_matrix_pos$mzmed[present_call[, 1]], na.rm = TRUE),
     ...
     xlab = "m/z", ylim = c(0, 0.0035), lwd = 2)
for (i in 2:4) {
...
The matrices are mostly alike. in the low mz all matrices follow one another.
At medium and high, the RBC and capillary, and the plasma and venous respectively, follow each other almost perfectly
...
missing in the plasma sample must be hydrophillic.
...
from 100 to 200, where fewer than expected are found.
...
plot(x = 1, y = 1, pch = NA, xlim = c(50, 1100), ylim = c(-150, 150),
...
for (src in TestedSourcenames) {
    ...
    yLow <- hist(yLow, breaks = seq(0, 350, 5), plot = FALSE)   
    yHigh <- res_matrix_pos[
        which(res_matrix_pos[, paste0("significant.source", src)] & 
              res_matrix_pos[, paste0("coef.source", src)] > 0), "rtmed"]
    yHigh <- hist(yHigh, breaks = seq(0, 350, 5), plot = FALSE)
    lines(yHigh$mids, yHigh$counts, col = paste0(col_source[src], 80), lwd = 2)
    lines(yLow$mids, -yLow$counts, col = paste0(col_source[src], 80), lwd = 2)
}
grid()
...
The m/z plot is almost identical, with the sole difference of the low m/z range.
Here almost no significant features were found in the original data. With the
grouped data this is the diminating region, even necessitating an expansion of
the y-limits. It is unclear why we see this difference
The rt plot is mostly the sam,e with slightly less underrepresentated RBC
features

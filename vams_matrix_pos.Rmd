...
The features present in either all matrices, or all of a specific matrix is defined.
...
assays(vams_pos)$Imputed <- (imputeRowMinRand( assay(vams_pos), min_fraction = 1/2, sd_fraction = 1, abs = TRUE))

```

The chromatograms of these common features are then made.
...
```{r BoxplotOfAbundances, fig.cap = "The average abundances of each of the features within a matrix. The boxplots show that although plasma is generally a bit higher the medians, and quatiles for all matrices are comparable"}
...
intsPresent <- list(
  common =  rowMeans(imputeRowMinRand( assays(vams_pos)$Imputed[feats_common,]
                               , min_fraction = 1/2, sd_fraction = 1, abs = TRUE)), 
  
  plasma = rowMeans(imputeRowMinRand( assays(vams_pos)$Imputed[feats_plasma,
    vams_pos$source == "plasma"], min_fraction = 1/2, sd_fraction = 1, abs = TRUE)),
    
    venous = rowMeans(imputeRowMinRand( assays(vams_pos)$Imputed[feats_venous,
    vams_pos$source == "venous"], min_fraction = 1/2, sd_fraction = 1, abs = TRUE)),
    
    RBC = rowMeans(imputeRowMinRand( assays(vams_pos)$Imputed[feats_RBC,
    vams_pos$source == "RBC"], min_fraction = 1/2, sd_fraction = 1, abs = TRUE)),

    capillary = rowMeans(imputeRowMinRand( assays(vams_pos)$Imputed[feats_capillary,
...
par(mar = c(7, 4.5, 1, 1))
boxplot(lapply(intsPresent, log2), varwidth = TRUE, col = col_source[names(intsPresent)],
        ylab = expression(log[2]~abundance), las = 2, main = "average abundance")
```
...
Boxplots are best for finding large differences, so a cumulative
distribution function was made in addition for each of the matrices.
Showing whether the distributions are somewhat identical.

```{r CumulativeDistributionFunctionsOfAbundances, fig.cap = "Cumulative density function of each matrix. The distributions of the matrices are mostly identical, with the plasma having a sligthly higher overall abundance and the RBC a slightly higher proportion of low abundance features"}
...
plasma_ecdf <- ecdf(as.numeric(log2(intsPresent$plasma)))
venous_ecdf <- ecdf(as.numeric(log2(intsPresent$venous))) 
RBC_ecdf <- ecdf(as.numeric(log2(intsPresent$RBC)))
capillary_ecdf <- ecdf(as.numeric(log2(intsPresent$capillary)))
...
the RBC is the lowest,and the plasma is the highest, the venous and capillary are comparable.
...
be much smaller than the earlier PCA plot, where matrix specifc features would 
naturally be a large part of the variation seen.

```{r PCAOfAbundances, fig.cap = "PCA of all common features by abundance. In plot showing the 1st and 2nd dimension, 6 outliers are found. Some difference by martrix is seen as well, with the capillary being separated, this is also seen in the 3rd dimension."}
...
pc <- prcomp(t(assays(vams_pos)$Imputed[feats_common,]), scale = FALSE, center = TRUE)
pchs <- ifelse(vams_pos$sex == "F", 21, 22)
...
```{r OutlierFilteredPCA, fig.cap = "PCA of abundances of all common features, with FT08876 and FT08384 removed. Only the plasma samples separate from the other samples, in the 2nd dimension"}
...
fitlmFunction <- function(x, dataMat, model = conc ~ source, weights = NULL){
...
fitMatrix <- t(apply(Data[, , drop = FALSE], 1, fitlmFunction, 
  dataMat = colData(vams_pos), model = conc ~ source + sex))

p.adjusted <- apply(fitMatrix[,paste0("pvalue.", SourcesTypes)], 2, p.adjust, 
                    method="BH")
#' The fitMatrix is appened to the rest of the data in vams_pos, making the new
#' dataframe res_Matrix_pos
...
par(mfrow=c(1, 3))
for (i in 1:3) {
hist(res_Matrix_pos[, paste0("pvalue.", SourcesTypes[i])], main = 
       #' paste0("Histogram of ", TestedSourcenames[i], " pvalues"), xlab = "pValue")
}
...
colorFilterLowP <- -apply(res_Matrix_pos[, paste0("adj.pvalue.", SourcesTypes)], 2, 
                          log10) > pLimit
...
#' The significant samples (both high fold difference and low p-value) are defined
sign <- abs(fitMatrix[, grep("coef", colnames(fitMatrix))]) > foldLimit & p.adjusted < pLimit
#' replace the "coef" in the column names with "significant"
colnames(sign) <- sub("coef", "significant", colnames(sign))
 ...
 par(mfrow=c(1, 3))
  for (i in 1:3){
    plot(res_Matrix_pos[,paste0("coef.", SourcesTypes[i])],  cex = 0.6,
         ...
         pch = 16, main = paste0("Capillary VS. ", TestedSourcenames[i] ), 
         col = col_volcano_Trans[colorFilter[, i]], 
         ylab = "-log10, Benjamini-Hochberg adjusted p-value", 
         xlab = "log2 fold change")
  abline(h=pLimit)
  abline(v=foldLimit)
  abline(v=-foldLimit)
  legend("top", c("non-Significant", "Highfold", "lowP","Significant"),
         col = col_volcano, pch = 16)
  }
...
pandoc.table(apply(colorFilter == 4, 2, sum, na.rm=TRUE), style = "rmarkdown",
             caption = cpt)
```
...
```{r pValueDependencyOnmzNoconfounding, fig.cap = "Distribution of m/z values for detected features in a matrix.". Apart from the plasma, all the matrices share roughly the same distribution"}
...
plot(density(res_Matrix_pos$mzmed[present_call[, 1]], na.rm=TRUE),
     col = col_source[colnames(present_call)[1]], main = "Distribution of mz-values",
     xlab="mz", ylim = c(0,0.002))
for (i in 2:4) {
lines(density(res_Matrix_pos$mzmed[present_call[, i]], na.rm=TRUE), 
      col = col_source[colnames(present_call)[i]])
}
...
Looking at the distribution of all features as a functionof the mz, only the
plasma is clearly different from the others. Here a higher proportion of medium-mz,
and much fewer high-mz are found. This is not surprising. When making plasma
high-weight compounds are removed from the matrix.
...
plot(density(res_Matrix_pos$mzmed[], na.rm=TRUE), 
     col = col_source["capillary"], main = "Distribution of mz-values,
     significantly different from capillary", xlab = "mz", ylim = c(0,0.002))
for (i in 1:3) {
lines(density(res_Matrix_pos$mzmed[colorFilterLowP[, paste0("adj.pvalue.",
    SourcesTypes[i])]], na.rm=TRUE), col = col_source[TestedSourcenames[i]])
}
legend("topleft", legend=c("All features", TestedSourcenames), 
       col = col_source[c("capillary", TestedSourcenames[1:3])], pch = 16)

```

This distribution includes only the features show to be significantly different
in the previous model. The overall shape is mostly the same.
Most features are found in the range with the most features overall. 
Interestingly the features from venous, have a larger proportion in the medium-low range.
...
the high range, as the matrix is missing many of these features alltogether.

As the previous plots does not show whether a feature is over- or underrepresentated 
when compared to the capillary sample a new plot was made. This shows the
distributions split by whether they are up- and downrepresentated in the matrix
compared to capillary.

```{r mzCountsSignificant, fig.cap = "Up- or downregulation of significant features. The up- or downregulation of significant features as a function of the retention time. Of note is how well the venous and plasma conform in upregulation in the 200-600 range and how significantly the plasma is downregulated compared to the capillary in the 600-800 range."}

par(mfrow=c(1, 1))
plot(x = 1, y = 1, pch = NA, xlim = c(50,1100), ylim = c(-100, 100), main =
    paste0("RT, up- or downregulation, significant pval - vs capillary "),
    xlab = "mz", ylab = "count")

for (src in TestedSourcenames) {
yLow <- res_Matrix_pos[which(res_Matrix_pos[, paste0("significant.source", src)] & 
  res_Matrix_pos[, paste0("coef.source", src)] < 0), "mzmed"]
yLow <- hist(yLow, breaks = seq(50, 1100, 5), plot = FALSE)   

yHigh <- res_Matrix_pos[which(res_Matrix_pos[, paste0("significant.source", src)] & 
  res_Matrix_pos[, paste0("coef.source", src)] > 0), "mzmed"]
yHigh <- hist(yHigh, breaks = seq(50, 1100, 5), plot = FALSE)
    
lines(yHigh$breaks[-1], yHigh$counts, col = col_source[src])
lines(yLow$breaks[-1], -yLow$counts, col = col_source[src])
}
abline(h = 0)
...
24 Da, corresponding to about 2 residues in the acyl chain of a fatty acid.


Likewise the distributions of the samples as a funcion of the rentention time was 
made.

```{r pvalueDependencyOnRetentionTimesNoConfounding, fig.cap = "Distribution of features as a function of retention time. The matrices mostly conform to one anothers distributions, with the exception of plasma. Plasma has major differences across the entire range. Likewise Venous has a higher density in the 150-175 sec range, but otherwise conforms."}
#' Distributions of retention times for each matrix.
plot(density(res_Matrix_pos$rtmed[present_call[, 1]], na.rm = TRUE), col =
       col_source[colnames(present_call)[1]], main="rt, values", 
     xlab = "Retention time (sec)", ylim = c(0,0.016))
for (i in 2:4) {
...
The plasma have a lower overall proportion of long retention time features and a
...
The opposite is seen, features with a long retention time, i.e. being hydrophillic,
have a lower presence, thus whatever is missing in the plasma sample must be both
...
difference is seen for the matrices. Although a large amount of the features have
low retention times, a comparatively smaller amount is significantly different here.
with especially the venous being virtually identical to the capillary
Most differences are found in the medium to medium-long retention times.
The venous especially are found to be different in the medium range. 
...
Once again a plot showing the distributions split by whether they are up- or
...
plot(x = 1,y = 1, pch = NA, xlim = c(15.7, 252), ylim = c(-350, 350),
     main = paste0("Rt, significant pval - vs capillary "), 
      xlab = "Retention time", ylab = "count")
for (src in TestedSourcenames) {
yLow <- res_Matrix_pos[which(res_Matrix_pos[, paste0("significant.source", src)] & 
  res_Matrix_pos[, paste0("coef.source", src)] < 0), "rtmed"]
yLow <- hist(yLow, breaks = seq(0,250, 2), plot = FALSE)   

yHigh <- res_Matrix_pos[which(res_Matrix_pos[, paste0("significant.source", src)] & 
  res_Matrix_pos[, paste0("coef.source", src)] > 0), "rtmed"]
yHigh <- hist(yHigh, breaks = seq(0,250, 2), plot = FALSE)
...
is both over- and underrepresented in plasma, but only underrepresented in the RBC.
Venous is moderate in the entire range.

The medium range is even more interesting. Here, the venous follows either the
RBC or the plasma closely. e.g. at around 160 sec the plasma and venous are 

...
author: "Johannes Rainer, Søren Fjelstr, Chiara Volani and Giuseppe Paglia"
graphics: yes
output:
...
```
Feature FT8876 and feature FT08384 were found to be responsible for the 7 outlier
samples and were filtered away.
...
Multiple linear regression of the features common for all samples are made.
A volcano plot for each of the combination, of plasma compared to all the other sources were made.
...
fitlmFunction <- function(x,dataMat){
  ...
  names(Output) <- c(paste0("coef.",c("capillary","RBC","venous")), paste0("pvalue.", c("capillary","RBC","venous")))
  Output
}
...
par(mar = rep(2, 4))
pLimit <- -log10(0.01/length(fitMatrix[,"coef.capillary"]))
foldLimit <- 1
...
colorFilterHighFold <- abs(fitMatrix[,1:3,drop=FALSE]) > foldLimit
...
#' The sums and proportion of points conforming to low p, and low p and high fold are found Missing values!

par(mar = rep(2, 4))
#' The volcano plots are made.
plot(fitMatrix[,"coef.capillary"], -log10(fitMatrix[,"pvalue.capillary"]),main="Plasma VS. Capillary", col=col_volcano[colorFilter[,1]],ylab = "-log10 , Bonferroni  adjusted p-value",xlab = "log2 fold change")
abline(h=pLimit)
abline(v=foldLimit)
abline(v=-foldLimit)

plot(fitMatrix[,"coef.RBC"], -log10(fitMatrix[,"pvalue.RBC"]),main="Plasma VS. RBC", col=col_volcano[colorFilter[,2]],ylab = "-log10 , Bonferroni  adjusted p-value",xlab = "log2 fold change")
abline(h=pLimit)
abline(v=foldLimit)
abline(v=-foldLimit)

plot(fitMatrix[,"coef.venous"], -log10(fitMatrix[,"pvalue.venous"]),main="Plasma VS. Venous", col=col_volcano[colorFilter[,3]],ylab = "-log10 , Bonferroni  adjusted p-value",xlab = "log2 fold change")
abline(h=pLimit)
abline(v=foldLimit)
abline(v=-foldLimit)
...
```
```{r Chromatogram_Missing_values, eval=FALSE}
load("data/RData/vams_normalization/data_pos.RData")

dr <- paste0(IMAGE_PATH, "Missing values plots/")
dir.create(dr, showWarnings = FALSE)
col_sample <- col_source[data_pos$source]
plot_feats <- function(x, col) {
    col_bg <- paste0(col[chromPeaks(x)[, "sample"]], 20)
...
idx <- which(is.na(rowSums(fitMatrix)))
feats <- names(idx)
feats_chrs <- featureChromatograms(data_pos, expandRt = 4, features = feats)
for (i in seq_along(feats)) {
    fn <- paste0(dr, feats[i], ".png")
...
A linear model comparing each of the samples to plasma was made. Accounting for 
age,sex,batch, and donor.

A volcano plot for each comparison were made.
This shows thatof the three sources venous is the most alike to the plasma
samples, with a smaller proportional of feautures being different.

Likewise it was decided to perform a PLS analysis of the data. 
Imputation was needed due to the many missing values.


```{r PLS}

#' Colorsource is redefined to ensure correct color



#' The ratio of the training set is calculated, and the training and test set is made
...
#' The data is defined and imputed to remove missing values (Otherwise it would drop rows causing errors)
#' Data <- ints$common[,,drop=FALSE]
Data <- (assay(vams_pos, "abundances"))
Data <- imputeRowMinRand(Data)
tData <- t(Data)


#' The Data and responses for the training and test sets are defined
...
#' The data is preprocessed, and the Model is made using the training data.
#' The model is made with the max number of components and 10-fold random 
#' segmented cross validation to find the correct number of components.
...
#' A confusion matrix with the model is made.
...
#' The PRESS VS number of components is plotted. this shows a minimum at 5-6 Components
...
#' A list of the means of the predictions (in practice the "probability") for 
#' each source is made.along with 2 times the standard deviation.

Mean <- list(
  capillary = apply(useSoftmax$fitted.values[trainSource=="capillary",,BstNComp],2,mean),
  
  plasma = apply(useSoftmax$fitted.values[trainSource=="plasma",,BstNComp],2,mean),
  
  RBC = apply(useSoftmax$fitted.values[trainSource=="RBC",,BstNComp],2,mean),
  
  venous  = apply(useSoftmax$fitted.values[trainSource=="venous",,BstNComp],2,mean)
)

StdDev <- list(
  capillary = 2*apply(useSoftmax$fitted.values[trainSource=="capillary",,BstNComp],2,sd),
  
  plasma = 2*apply(useSoftmax$fitted.values[trainSource=="plasma",,BstNComp],2,sd),
  
  RBC = 2*apply(useSoftmax$fitted.values[trainSource=="RBC",,BstNComp],2,sd),
  
  venous  = 2*apply(useSoftmax$fitted.values[trainSource=="venous",,BstNComp],2,sd)
)
  


#' The mean of the scores is plotted along with shading corresponding to 2 times the standard deviation.
plot(1:4,Mean$capillary,xlim=c(1,4),ylim=c(-0.2,1.2),type="n",main="Mean of scores"
     ,axes = FALSE,ylab = "Probability Score", xlab="")
...
#' The final model is made using the limited number of components.
#' A confusion matrix is included

for (i in 1:length(trainSource)) {
points(1:4,t(useSoftmax$fitted.values[i,,BstNComp]),col=col_source[trainSource[i]])
}

useSoftmax <- plsda(trainDescr, trainSource, ncomp = BstNComp,validation = "CV")
...
```
80% of the samples are used for training, with the remaining 20%  used for
indedpendent validation. The assignment is random.
The training of the PLS is performed with and n=N-1 components and 10 segments 
these are likewise chosen randomly.
A confusion matrix of the prediction using the model on the test set is produced,
together with a graph detailing the PRESS as a function of components used. 
2 Fewer than the component in which the minima is reached is used *(This is done*
*manually, but find way to do so automatically)* This is currently 6 components. 
A new model using  6/10 components are made. The new model is tested, producing a 
plot showing the probability scores of the samples *(Find a way to ensure they*
*are normalized to [0,1])* and a new confusion matrix.
The probability scores were colored according to source and shading corresponding
to two standard deviations were includedThis confusion matrix shows that a
identification of 100% is possible using PLS, even without adjusting for confounding factors.
...
#' The new data set is defined. Any missing value is given the value 1, in contrast
#' a *nonmissing* value is given the value 0.
MissingValues <- 1*is.na(assay(vams_pos, "abundances"))


#' A PCA plot with this new dataset is performed
...
#' The linear model fit function is restated here. remove in the future
fitlmFunction <- function(x,dataMat){
  dataMat <- cbind(conc=x,as.data.frame(dataMat))
  dataMat$source <- factor(x = vams_pos$source,levels = c("plasma","capillary", "RBC", "venous"))
...
#' The density of the aggregated means are plotted. This is effectively a measure
#' of how abundant the missing values are.
par(mfrow= c(1,2))
plot(density(t(MissingValues[1,])),col=col_source[2],ylab="Density", xlab = expression(log[2]~abundance), main = "Density function of abunbance")
lines(density(t(MissingValues[2,])),col=col_source[1])
lines(density(t(MissingValues[3,])),col=col_source[3])
lines(density(t(MissingValues[4,])),col=col_source[4])


#' Since most values are not missing a zoomed in plot exluding the vast majority in x=0 is made.
plot(density(t(MissingValues[1,-1])),col=col_source[2],xlim=c(0,0.2),ylim=(c(0,5)),ylab="Density", xlab = expression(log[2]~abundance), main = "Density function of abunbance - Zoomed")
lines(density(t(MissingValues[2,])),col=col_source[1])
lines(density(t(MissingValues[3,])),col=col_source[3])
lines(density(t(MissingValues[4,])),col=col_source[4])
...
blood. The third dimension can discriminate between RBC plasma and capillary with

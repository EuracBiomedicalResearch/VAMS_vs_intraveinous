...
```{r sex_neg_pvalue_hist, fig.path = IMAGE_PATH, echo = FALSE, fig.cap = "Histogram of the p-values for male vs female in each of the matrices. Analysis was performed on the full data.", fig.width = 12, fig.height = 4}
par(mfrow = c(1, 4))
for (src in TestedSourcenames) {
...
```{r sex_neg_volcano, fig.path = IMAGE_PATH, echo = FALSE, fig.cap = "Volcano plots representing the results for the comparison between male and female individuals. Analysis was performed on the full data.", fig.width = 12, fig.height = 4}
par(mfrow = c(1, 4))
...
```{r sex_neg_sig_table, results = "asis", echo = FALSE}
sig_count <- rowSums(
...
```{r sex_neg_sig_ann, echo = FALSE, eval = FALSE}
library(CompoundDb)
...
```{r sex_neg_top_50_pval, echo = FALSE, fig.path = IMAGE_PATH, fig.cap = "Overlap of top 50 features with smallest p-values (male vs female)."}

...
tmp_pool <- res_pos[, res_pos$sample == "POOL"]
mdls_batch <- xcms:::rowFitModel(y ~ inj_idx,
                                 data = as.data.frame(colData(tmp_pool)),
                                 y = log2(assay(tmp_pool, "within_nofill")),
                                 method = "lm", minVals = 10)

flgs_inj_range <- vapply(mdls_batch, flag_model_inj_range, logical(1),
                         min_range = diff(range(res_pos$inj_idx)) * req_prop)

## Plot the distribution of slopes.
par(mar = c(4, 4.5, 1, 0.5))
hist(slps_batch, breaks = 128, xlab = "slope", main = "Distribution of slopes")
if (any(flgs_inj_range_a, na.rm = TRUE))
    hist(slps_batch[which(flgs_inj_range)], breaks = 128, add = TRUE,
         col = "#0000ff80")
abline(v = 0)

## Split into excluded and good mdls
mdls_inj_range_a <- mdls_batch[which(flgs_inj_range)]

## Remove model fits for the flagged friends.
mdls_batch[unique(which(flgs_inj_range))] <- NA
slps_batch[unique(which(flgs_inj_range))] <- NA
...
dir.create(dr, showWarnings = FALSE)
fts <- names(mdls_batch)[which(abs(slps_batch) > 0.025)][1:20]
plot_feature <- function(x, y, model, ...) {
  plot(x, y, ...)
  if (length(model) > 1)
    abline(model)
...
plot_slopes <- function() {
  for (ft in fts) {
    png(paste0(dr, ft, ".png"), width = 10, height = 5, pointsize = 4,
        res = 200, units = "cm")
    plot_feature(x = res_pos$inj_idx,
                 y = log2(assay(res_pos, "within_filled")[ft, ]),
                 model = mdls_batch[[ft]],
                 pch = ifelse(is.na(assay(res_pos, "raw")[ft, ]),
                              yes = 1, no = 16),
                 col = col_source[as.character(res_pos$source)], main = ft,
...
```{r boxplot-after-within-normalization, fig.path = IMAGE_PATH, fig.height = 7, fig.width = 7 * phi, fig.cap = "Comparison between abundances (log2) of raw data, median-scaled data and normalized data.", message = FALSE, echo = FALSE}
##Raw data
...
```{r boxplot-after-within-norm-good-is, fig.path = IMAGE_PATH, fig.height = 7, fig.width = 7 * phi, fig.cap = "Comparison between abundances (log2) of raw data, median-scaled data and normalized data of Internal Standards.", message = FALSE, echo = FALSE}
##Raw data
par(mfrow = c(4, 1), mar = c(0.2, 4.5, 4.5, 0.5))
cols <- col_source[as.character(res_pos$source)]
...
```{r rla-after-between-normalization, fig.path = IMAGE_PATH, fig.height = 7, fig.width = 7 * phi, fig.cap = "RLA of raw data, median-scaled data and normalized data.", message = FALSE, echo = FALSE}
## Raw data
par(mfrow = c(4, 1), mar = c(0.2, 4.5, 4.5, 0.5))
...
```{r rla-after-within-normalization-is, fig.path = IMAGE_PATH, fig.height = 7, fig.width = 7 * phi, fig.cap = "Final RLA showing a comparison between raw data and normalized data of Internal Standards.", message = FALSE, echo = FALSE, warning = FALSE}
## Raw data
...
tmp <- assay(res_pos, "normalized_filled")
tmp[is.na(assay(res_pos, "raw"))] <- NA
rsd_bbn_det <- rowRsd(tmp[, res_pos$sample == "POOL"])
```
...
## Calculate RSD on problematic internal standards in POOL signals
tmp_is <- assay(res_pos, "raw_filled")[is_std_prob$feature_id, ]
rsd_raw_is_prob <- rowRsd(tmp_is[, res_pos$sample == "POOL"])
tmp_is <- assay(res_pos, "median_filled")[is_std_prob$feature_id, ]
rsd_scl_is_prob <- rowRsd(tmp_is[, res_pos$sample == "POOL"])
tmp_is <- assay(res_pos, "within_filled")[is_std_prob$feature_id, ]
rsd_adj_is_prob <- rowRsd(tmp_is[, res_pos$sample == "POOL"])
tmp_is <- assay(res_pos, "normalized_filled")[is_std_prob$feature_id, ]
rsd_bbn_is_prob <- rowRsd(tmp_is[, res_pos$sample == "POOL"])

## Calculate RSD on internal standards in study samples
tmp_is <- assay(res_pos, "raw_filled")[is_std_prob$feature_id, ]
rsd_raw_is_prob_sts <- rowRsd(tmp_is[, res_pos$sample != "POOL"])
tmp_is <- assay(res_pos, "median_filled")[is_std_prob$feature_id, ]
rsd_scl_is_prob_sts <- rowRsd(tmp_is[, res_pos$sample != "POOL"])
tmp_is <- assay(res_pos, "within_filled")[is_std_prob$feature_id, ]
rsd_adj_is_prob_sts <- rowRsd(tmp_is[, res_pos$sample != "POOL"])
tmp_is <- assay(res_pos, "normalized_filled")[is_std_prob$feature_id, ]
rsd_bbn_is_prob_sts <- rowRsd(tmp_is[, res_pos$sample != "POOL"])
```
...
T <- rbind(`mean RSD` = c(raw = mean(rsd_raw, na.rm = TRUE),
                          ...
                          `within-batch detected` = mean(rsd_adj_det,
                                                       na.rm = TRUE),
                          `between-batch detected` = mean(rsd_bbn_det,
                                                       na.rm = TRUE),

...
tmp_perc <- apply(fsumm[, paste0(study_src, "_perc")], 1,
                  function (z) any(z > 30))
fts_used <- rownames(
    featureDefinitions(data_pos))[which(tmp_perc)]
...
```{r fill-in, message = FALSE, warning = FALSE, eval = !file.exists(paste0(RDATA_PATH, "data_pos.RData"))}
rtwidths <- chromPeaks(data_pos)[, "rtmax"] - chromPeaks(data_pos)[, "rtmin"]
...
assays(res_pos)$raw_filled_imputed <- imputeRowMinRand(
                   assay(res_pos, "raw_filled"), method = "from_to")
```
...
each other. Ideally, the filled-in signal should be correlated to the detected
signal.
...
barplot(apply(assay(res_pos, "raw"), MARGIN = 2, function(x) sum(!is.na(x))),
        ...
        xaxt = "n")
dobox(log2(assay(res_pos, "raw")), xaxt = "n")
...
barplot(apply(assay(res_pos, "raw_filled"), MARGIN = 2,
        ...
        col = paste0(col_source[data_pos$source], 80), ylab = "features",
        xaxt = "n")
dobox(log2(assay(res_pos, "raw_filled")), xaxt = "n")
...
boxplot(rowRla(assay(res_pos, "raw"), data_pos$source), cex = 0.5, pch = 16, 
...
boxplot(rowRla(assay(res_pos, "raw_filled"), data_pos$source),
        cex = 0.5, pch = 16, col = col_source[data_pos$source], ylab = "RLA",
        border = paste0(col_source[data_pos$source], 40), notch = TRUE,
...
for (i in seq(nrow(is_info))) {
    ...
    is_info$mz_ion[i] <- unlist(
        mass2mz(is_info$mzneut[i],
                adduct = as.character(
                    is_info[i, grep("POS", colnames(is_info))])))
}
...
is_info$feature_id[4] <- NA_character_
is_info$feature_id[9] <- NA_character_
is_info$feature_id[10] <- NA_character_
is_info$feature_id[11] <- NA_character_
is_info$feature_id[14] <- NA_character_
is_info$feature_id[15] <- NA_character_
is_info <- is_info[!is.na(is_info$feature_id), ]
...
tmp_imp <- assay(res_pos, "raw_filled_imputed")
...
similar separation is visible for the analysis on internal standards on PC1. 
...
#' sum of signal.
sms <- colSums(assay(res_pos, "raw"), na.rm = TRUE)
nf_sm <- sms / median(sms)

#' median signal
mdns <- colMedians(assay(res_pos, "raw"), na.rm = TRUE)
nf_mdn <- mdns / median(mdns)

#' MRM
nf_mrm <- estimateSizeFactorsForMatrix(assay(res_pos, "raw"))

#' Normalize
fv_sm <- sweep(assay(res_pos, "raw_filled_imputed"), MARGIN = 2, nf_sm, `/`)
fv_mdn <- sweep(assay(res_pos, "raw_filled_imputed"), MARGIN = 2, nf_mdn, `/`)
fv_mrm <- sweep(assay(res_pos, "raw_filled_imputed"), MARGIN = 2, nf_mrm, `/`)
...
fv_nms <- 2^t(NormQcmets(
                t(log2(assay(res_pos, "raw_filled_imputed"))), method = "nomis",
                qcmets = which(rownames(res_pos) %in% is_info$feature_id)
            )$featuredata)
#' re-add the internal standards!
fv_nms <- rbind(
    fv_nms,
    assay(res_pos, "raw_filled_imputed")[is_info$feature_id, ])
fv_nms <- fv_nms[rownames(res_pos), ]
...
fts_int_std_mean <- colMeans(assay(res_pos, "raw")[is_info$feature_id, ],
                             na.rm = TRUE)

cor_int_std <- apply(log2(assay(res_pos, "raw")),
                     MARGIN = 1, cor, y = log2(fts_int_std_mean))

neg_ctrls <- which(cor_int_std > 0.6)
neg_ctrls_int <- which(rownames(res_pos) %in% is_info$feature_id)
...
ruv_rand <- NormalizeRUVRand(t(log2(assay(res_pos, "raw_filled_imputed"))),
                             ctl = neg_ctrls_int, k = 5,
                             plotk = FALSE)
fv_ruvr <- 2^t(ruv_rand$newY)

library(ruv)
ruv_3 <- RUVIII(t(log2(assay(res_pos, "raw_filled_imputed"))),
                ctl = neg_ctrls_int, k = 5,
                M = design.matrix(res_pos$sample))
fv_ruv3 <- 2^t(ruv_3)
...
mra_raw <- mra_for_mat(assay(res_pos, "raw_filled"),
                       assay(res_pos, "raw"), grp, "all-POOL")
mra_sm <- mra_for_mat(fv_sm, assay(res_pos, "raw"), grp, "all-POOL")
mra_mdn <- mra_for_mat(fv_mdn, assay(res_pos, "raw"), grp, "all-POOL")
mra_mrm <- mra_for_mat(fv_mrm, assay(res_pos, "raw"), grp, "all-POOL")
mra_ruvr <- mra_for_mat(fv_ruvr, assay(res_pos, "raw"), grp, "all-POOL")
mra_ruv3 <- mra_for_mat(fv_ruv3, assay(res_pos, "raw"), grp, "all-POOL")
mra_nms <- mra_for_mat(fv_nms, assay(res_pos, "raw"), grp, "all-POOL")
```
...
par(mfrow = c(7, 1), mar = c(.5, 4.5, 1, 0.1))
dobox(log2(assay(res_pos, "raw_filled_imputed")), main = "raw data", ylim = c(5, 20))
points(colMeans(log2(assay(res_pos, "raw_filled_imputed")), na.rm = TRUE), type = "l")
...
rsd_raw <- rowRsd(assay(res_pos, "raw_filled_imputed")[, res_pos$sample == "POOL"])
rsd_sm <- rowRsd(fv_sm[, res_pos$sample == "POOL"])
rsd_mdn <- rowRsd(fv_mdn[, res_pos$sample == "POOL"])
rsd_mrm <- rowRsd(fv_mrm[, res_pos$sample == "POOL"])
rsd_ruvr <- rowRsd(fv_ruvr[, res_pos$sample == "POOL"])
rsd_ruv3 <- rowRsd(fv_ruv3[, res_pos$sample == "POOL"])
rsd_nms <- rowRsd(fv_nms[, res_pos$sample == "POOL"])
...
dependent signal drift to abundances **separately** for the two batches. The
advantage of the separate analysis is that the analysis in one batch is not
dependent on the number of valid measurements of the feature also in the
other batch. Below we fit linear models `y ~ inj_idx` to the log2 transformed 
abundances (only of detected peaks) using robust regression [@Koller:2017jsa]. 
Model fitting is skipped for features with less than 12 valid signals. We fit 
the linear models to the full data instead of only the QC samples as they do 
not seem to be representative of all matrices (data not shown). This obviously
requires properly randomized samples.

```{r fit-model-batch, message = FALSE, warning = FALSE}
smpls_a <- res_pos$batch == "31012018"
mdls_a <- xcms:::rowFitModel(
                     y ~ inj_idx,
                     data = as.data.frame(colData(res_pos))[smpls_a, ],
                     y = log2(assay(res_pos, "norm_nofill")[, smpls_a]),
                     method = "lmrob", minVals = 12)

smpls_b <- data_pos$batch == "01022018"
mdls_b <- xcms:::rowFitModel(
                     y ~ inj_idx,
                     data = as.data.frame(colData(res_pos))[smpls_b, ],
                     y = log2(assay(res_pos, "norm_nofill")[, smpls_b]),
                     method = "lmrob", minVals = 12)
...
flgs_inj_range_a <- vapply(mdls_a, flag_model_inj_range, logical(1),
                           min_range = diff(range(res_pos$inj_idx)) * 3/4)
...
flgs_inj_range_b <- vapply(mdls_b, flag_model_inj_range, logical(1),
                           min_range = diff(range(res_pos$inj_idx)) * 2/3)
...
for (ft in fts) {
    png(paste0(dr, ft, ".png"), width = 14, height = 7, pointsize = 6,
        res = 200, units = "cm")
    plot_feature_slopes_batch(y = log2(assay(res_pos, "norm")[ft, ]),
...
tmp[, smpls_b] <- xcms:::applyModelAdjustment(
                               ...
                               data = as.data.frame(colData(res_pos))[smpls_b, ],
                               shiftNegative = "replaceHalfMin")
assays(res_pos)$norm <- 2^tmp
tmp[is.na(assay(res_pos, "raw"))] <- NA
assays(res_pos)$norm_nofill <- 2^tmp
...
for (ft in fts) {
    ...
    png(paste0(dr, ft, "_adj.png"), width = 14, height = 7, pointsize = 6,
        res = 200, units = "cm")
    y_sub <- log2(assay(res_pos, "norm_nofill")[ft, ])
...
knitr::include_graphics(paste0(dr, fts[2], "_raw.png"))
```
...
across QC samples. Note that evaluating normalization performance on the RSD of
QC samples might to some extend be biased, since they are also included in the
estimation of the injection index-dependent signal drift.

```{r within-batch-rsd, results = "asis"}
rsd_raw <- rowRsd(assay(res_pos, "raw_filled_imputed")[, res_pos$sample == "POOL"])
rsd_norm <- rowRsd(assay(res_pos, "norm")[, res_pos$sample == "POOL"])
...
*Note:* column `"QC_rsd"` contains the rsd for all abundances across all QC
samples, while `"all_rsd"` contais the rsd for abundances of only detected peaks

...
```{r per-feature-difference-filled-detected, message = FALSE, fig.cap = "Difference (log2 scale) of detected against filled-in signal.", fig.path = IMAGE_PATH, fig.height = 4, fig.width = 4, echo = FALSE}
...
Note that the models describing the batch effect and injection
dependent signal drift is estimated on the detected peak data, i.e. prior to
...
Fitting the linear models describing an (log scale) injection index dependent
signal drift to the abundances of QC samples separately in each batch. The
advantage of performing the analysis separately in each batch is that the drift
can be adjusted also for features for which only in one of the two batches
enough valid measurements would be available. The rationale of the separate
analysis is that the signal drift seems to be batch dependent, thus a different
slope would have to be estimated for each batch anyways. Below we fit linear
models `y ~ inj_idx` to the log2 transformed abundances using robust regression
\cite{Koller:2017jsa}. Model fitting is skipped for features with less than 4
valid signals. 

```{r fit-model-batch, message = FALSE, warning = FALSE}
smpls_a <- data_pos$sample == "POOL" & data_pos$batch == 31012018
mdls_a <- xcms:::rowFitModel(y ~ inj_idx, data = pData(data_pos)[smpls_a, ],
                             y = log2(fv_nofill[, smpls_a]), method = "lmrob")

smpls_b <- data_pos$sample == "POOL" & data_pos$batch == 1022018
mdls_b <- xcms:::rowFitModel(y ~ inj_idx, data = pData(data_pos)[smpls_b, ],
                             y = log2(fv_nofill[, smpls_b]), method = "lmrob")
```

We are next re-evaluating the model fits and removing linear models for features
for which valid measurements do not span more than 2/3 of the injection index
range or that have a difference between the 25 and 75% percentile of residuals
larger than 1 (i.e. more than two-fold deviation from the linear fit).
...
flag_model_inj_range <- function(x, min_range = 1, column = "inj_idx") {
    if (length(x) > 1) {
        if (diff(range(x$x[, column])) < min_range)
            TRUE
        else FALSE
    } else NA
}
...
length(which(flgs_res_a))
flgs_inj_range_a <- vapply(mdls_a, flag_model_inj_range, logical(1),
                           min_range = diff(range(data_pos$inj_idx)) / 2)
...
drift are 0 suggesting that most features are not affected by such a shift.
...
```{r fit-model-table, message = FALSE, eacho = FALSE, results = "asis"}
...
rownames(tab) <- c("total features", "large residuls", "low inj idx range",
                   "valid model fits")
pandoc.table(tab, style = "rmarkdown",
             caption = paste("Numbers of features for which an injection",
                             "index dependent model could be fitted."))
```
...
plot_feature_slopes_batch <- function(y, yfill, data, lmoda, lmodb, main) {
    ## Two plots, one per batch.
    par(mar = c(0.5, 4.5, 1, 0), mfrow = c(1, 2))
    a <- which(data$batch == data$batch[1])
    y_a <- y[a]
...
for (ft in fts) {
    png(paste0(dr, ft, ".png"), width = 10, height = 5, pointsize = 6,
        res = 200, units = "cm")
    plot_feature_slopes_batch(y = log2(fv_nofill[ft, ]), yfill = log2(fv[ft, ]),
...
TODO 
- check features with good and bad F-statistic; eventually remove those with bad
  stat.
- Compare the effect on adjusted ones: check RSD in QC samples, difference
  between replicates.

```{r}
adjr <- vapply(mdls, function(z) {
    if (length(z) == 1) NA_real_
    else summary(z)$adj.r.squared
}, numeric(1))

## 40 with poorest R squared
dr <- paste0(IMAGE_PATH, "lowest_R_squared/")
dir.create(dr, showWarnings = FALSE)
fts <- names(sort(adjr))[1:20]

for (ft in fts) {
    png(paste0(dr, ft, ".png"), width = 10, height = 5, pointsize = 6,
        res = 200, units = "cm")
    plot_feature_slopes(y = log2(fv_nofill[ft, ]), yfill = log2(fv[ft, ]),
                        data = pData(data_pos), lmod = mdls[[ft]],
                        main = ft)
    dev.off()
}
## These guys are mostly those with slope ~ 0

dr <- paste0(IMAGE_PATH, "largest_R_squared/")
dir.create(dr, showWarnings = FALSE)
fts <- names(sort(adjr, decreasing = TRUE))[1:20]
...
## These are nice fits with slopes != 0

## Next approach. residual for detected peaks has to be lower than 1
mean_res <- vapply(mdls, function(z) {
    if (length(z) == 1) NA_real_
    else {
        det <- weights(z) == 1
        diff(quantile(residuals(z)[det], probs = c(0.25, 0.75), na.rm = TRUE))
    }
}, numeric(1))

dr <- paste0(IMAGE_PATH, "mean_res_05/")
dir.create(dr, showWarnings = FALSE)
fts <- names(mean_res)[which(mean_res > 1)]
...
## These are mostly those on which the fit used the filled-in peaks.
...
Fitting on only detected peaks seems to result in exaggerated estimates for the
signal drift (see figures below, filled circles represent detected peak signals,
open circles filled-in signal; the fit was performed on the green filled circles
(QC samples).

![](images/vams_preprocessing/largest_slopes/FT00012.png)

![](images/vams_preprocessing/largest_slopes/FT08699.png)

![](images/vams_preprocessing/largest_slopes/FT09128.png)

Fitting on all QC signals, also filled-in abundances, has also drawbacks and can
lead to similar overestimation of drifts, if few detected peaks and/or filled-in
peaks are available.




- Table with numbers of features that could be adjusted.
- Check models with steepest slopes in a and in b.

Before adjusting the data with the 



```{r}
## For quality checks, calculate:
## - MRA (maximum ratio of abundances).
## - RSD (relative standard deviation between replicates).
rowMra <- function(x) {
    mra <- function(z) {
        z <- z[!is.na(z)]
        if (length(z) > 1) {
...
rowRsd <- function(x) {
    apply(x, MARGIN = 1, function(z) {
        sd(z, na.rm = TRUE) / abs(mean(z, na.rm = TRUE))
    })
}

rsd_pool <- rowRsd(fv[, data_pos$source == "all"])
plot(density(rsd_pool, na.rm = TRUE))

mra_pool <- rowMra(fv[, data_pos$source == "all"])
plot(density(mra_pool, na.rm = TRUE))

## Calculate RSD for replicates in each source.
rsd_mat <- function(x, groups, ids) {
    grps <- unique(groups)
    res <- lapply(grps, function(z) {
        x_sub <- x[, groups == z, drop = FALSE]
        ids_sub <- ids[groups == z]
        ids_unq <- unique(ids_sub)
        tmp <- do.call(cbind, lapply(ids_unq, function(id) {
            rowRsd(x_sub[, ids_unq == id, drop = FALSE])
        }))
        colnames(tmp) <- ids_unq
        tmp
    })
    names(res) <- grps
    res
...
rsds_raw <- rsd_mat(fv, data_pos$source, data_pos$sample)
rsds_raw_mn <- lapply(rsds_raw, rowMeans, na.rm = TRUE)
...
- Alternative: Use lowess on QC samples to estimate an effect and adjust each
  sample with that.

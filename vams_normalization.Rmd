...
```{r between-batch-problematic-pool, echo = FALSE, fig.path = IMAGE_PATH, fig.cap = "Feature for which abundances in pooled samples relative to study samples differ between the batches"}
...
TODO:
- histogram of slopes
- top 20 features with largest slopes.

```{r, echo = FALSE}

dr <- paste0(IMAGE_PATH, "largest_slopes_between_batch/")
dir.create(dr, showWarnings = FALSE)
...
TODO: 
- clean up and res-structure a bit below.
- remove some of the potentially redundant/uninformative plots (problematic IS?)
- add also summaries for the between-batch normalization
...
```{r boxplot-after-within-norm-good-is, fig.path = IMAGE_PATH, fig.height = 7, fig.width = 7, fig.cap = "Comparison between abundances (log2) of raw data, median-scaled data and normalized data of Internal Standards.", message = FALSE, echo = FALSE}
##Raw data
par(mfrow = c(3, 1), mar = c(0.2, 4.5, 4.5, 0.5))
cols <- col_source[as.character(res_pos$source)]
...
rsd_adj <- rowRsd(
    assay(res_pos, "within_filled")[, res_pos$source == "venous"])
...
rsd_adj <- rowRsd(
    assay(res_pos, "within_filled")[, res_pos$source == "capillary"])
...
We next evaluate the performance separately for each sample-matrix (source).
...
rsd_adj <- rowRsd(
    assay(res_pos, "within_filled")[, res_pos$source == "plasma"])
...
```{r norm-rsd-plot-plasma, fig.path = IMAGE_PATH, message = FALSE, echo = FALSE, fig.height = 7, fig.width = 7, fig.cap = "Coefficient Of Variation (RSD) for Plasma."}
par(mfrow = c(2, 3))
boxplot(list(raw = rsd_raw, scaled = rsd_scl, within = rsd_adj),
...
rsd_adj <- rowRsd(
    assay(res_pos, "within_filled")[, res_pos$source == "RBC"])
...
```{r norm-rsd-plot RBC, fig.path = IMAGE_PATH, message = FALSE, echo = FALSE, fig.height = 7, fig.width = 7, fig.cap = "Coefficient Of Variation (RSD) - RBC."}
par(mfrow = c(2, 3))
boxplot(list(raw = rsd_raw, scaled = rsd_scl, within = rsd_adj),
...
```{r final-eval-boxplot, echo = FALSE, fig.path = IMAGE_PATH, fig.width = 8, fig.height = 4, fig.cap = "Comparison between replicated measurements."}
par(mfrow = c(1, 2))
boxplot(cbind(raw = repl_raw[, "R"], median = repl_med[, "R"],
              within = repl_within[, "R"]), main = "R",
        ylab = "R")
grid(nx = NA, ny = NULL)
boxplot(cbind(raw = repl_raw[, 5], median = repl_med[, 5],
              within = repl_within[, 5]), main = "90% log2FC",
        ylab = "90%~log2FC")
grid(nx = NA, ny = NULL)
```

...
used in RNAseq data normalization might be used instead. Note that we did
...
absolute residuals is larger than 0.5.
...
filled-in values) based on the estimated models. Adjustment resulted in three
...
cpt <- paste0("Distribution of per-replicate 75% MRA quantile for the raw ",
              "and within-batch normalized data and average count of ",
              "features with a more than 2-fold difference in abundance.")
pandoc.table(T, style = "rmarkdown", caption = cpt)
```

Within-batch adjustment did reduce the 75% MRAs slightly, especially the 75%
quantile and the value ranges. The impact of within-batch normalization is more
pronounced if we consider only features that were actually adjusted:
...
residuals that are smaller than 0.5. Features that do not fulfill this criteria
will be normalized with a global model.
...
plot_ft_int <- function(x, ft, ...) {
    pchs <- ifelse(is.na(fv_nofill[ft, ]), yes = 1, no = 16)
    plot(log2(x[ft, ]), main = ft, col = col_source[data_pos$source],
         pch = pchs)
...
```{r final-qa-boxplots, fig.path = IMAGE_PATH, fig.width = 14, fig.height = 9, fig.cap = "Boxplots representing the per-sample distribution of signal. Shown are log2 abundances before (upper panel) and after normalization (below)."}
...
```{r final-qa-rla, fig.path = IMAGE_PATH, fig.width = 14, fig.height = 9, fig.cap = "RLA plots before and after normalization."}

...
#' Normalize
fv_sm <- sweep(assay(res_pos, "raw"), MARGIN = 2, nf_sm, `/`)
fv_mdn <- sweep(assay(res_pos, "raw"), MARGIN = 2, nf_mdn, `/`)
fv_mrm <- sweep(assay(res_pos, "raw"), MARGIN = 2, nf_mrm, `/`)
...
dependent also on the number of valid measurements of the feature also in the
...
flgs_inj_range_a <- vapply(mdls_a, flag_model_inj_range, logical(1),
                           min_range = diff(range(data_pos$inj_idx)) * 3/4)
...
drift are close to 0 suggesting only a relatively low influence. Only about 70
...
filled-in values) based on the estimated models. Adjustment resulted in 18
...
for (ft in fts) {
    ...
    lmoda <- xcms:::fitModel(y ~ inj_idx, data = pData(data_pos)[smpls_a, ],
                             method = "lmrob", y = y_sub[smpls_a])
    lmodb <- xcms:::fitModel(y ~ inj_idx, data = pData(data_pos)[smpls_b, ],
...
The 75% MRA of replicated samples is between 1.2 and 2.5 which is quite
OK, meaning that most samples have a less than 2-fold difference between
replicated measurements. Within-batch normalization could reduce the MRA for
some replicate pairs.

...
SemiTargeted <- vams_pos[std_features[,"feature_id"],]
...
The data is then exported for use in the sex comparison analysis.
...
save(SemiTargeted,
file = paste0("data/RData/vams_normalization/", "SemiTargeted_pos.RData"))
...
```
The PCA above was performed on the identified features (about 120). Missing
...
almost identical to the ones made using the entire dataset.
...
The distribution of contained samples is interesting. The amount of features
found in capillary, RBC and venous are almost identical, with the plasma having
about 20% fewer. Interestingly the RBC has more features than the venous, which
might be due to relative dilution of the sample in venous.
...
defined.
Note that due to only one feature being unique for the venous sample, a mean,
instead of a rowmeans, were performed.
...
ints <- list(
    ...
    venous = mean(assay(SemiTargeted)[feats_venous,
...
res_matrix_pos<- cbind(rowData(SemiTargeted), fitMatrix, adj = p.adjusted)
```
...
The features significantly different between samples are likewise defined.
...
for (i in 1:3){
    plot(res_matrix_pos[, paste0("coef.", SourcesTypes[i])],  cex = 1.6,
...
As expected from the histograms, and the fuill dataset, most of the features are
...
Out of the 121 features, approximatively 50, 25 and 30% of the features for
...
before for the m/z, the capillary, RBC and venous are almost identical in the
distribution, with the plasma lacking the higher m/z features. However, for the
targeted almost no features with a m/z above 400 is found.

For the retention time, all matrices are almost identical, which contrasts with
the full dataset in which the plasma samples were very different. Mostly
features with a long retention time were found. In the full dataset almost
equivalent high and low retention times were recorded.
...
Looking at the significant features in the m/z, the venous is most akin to the
capillary, then plasma, then RBC. All differences are found at ~ 150, with a
bump for venous around 280, but no higher molecular weight features.

For retention time, again, the plasma and RBC are almost identical, with the
venous having a different profile. Most of the differences are found in the
higher retention time.
The data is now divided into whether the features have a higher or lower
coeffecient than capilary.
...
Since so few samples are significant, it is hard to say much for these plots.
The most striking is the high amount of lower abudance features for the plasma
in the ~ 160 m/z range. Likewise the lower abundance of the plasma in the high

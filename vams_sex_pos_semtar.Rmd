...
# Introduction
Having found the differences between each matrix in [vams_matrix_pos_semtar.Rmd](vams_matrix_pos_semtar.Rmd).
...
Note that this is performed with only a subset of already indentified features.
...
load("data/RData/vams_normalization/SemiTargeted_pos.RData")
...
res_matrix_pos <- rowData(SemiTargeted)
...
Surprisingly not a single feature are found to be significant. Some have 
fold changes above 2, but none have an adjusted p-value below 0.01.
...
res_matrix_pos_Conf <- rowData(SemiTargeted)
Data <- log2(assays(SemiTargeted)$Imputed)[, ]
...
a big effect of including it. Although we do find some effect.
...
for (src in TestedSourcenames) {
 ...
 ages <- split(SemiTargeted$age, SemiTargeted$sex)

 hist(ages$F, col=rgb(1,0.1,0.1,0.5),xlim=c(25,55),breaks = 30)
 hist(ages$M, col=rgb(0.1,1,0.1,0.5),xlim=c(25,55),breaks = 30, add= T)

```

The distributions mostly look the same, 
...
for (src in TestedSourcenames) {
  tableSums <- cbind(tableSums,
    sum(res_matrix_pos_Conf[, paste0("pvalue.", src)] < 0.01))
}
names(tableSums) <- TestedSourcenames
cpt <- paste0("Number of features with a nonadjusted pvalue below 0.01 detected
  in each matrix")
...
A surprisingly big difference is seen, plasma halves from 7 to 3 and venous
loses all significance, going from 7 to 0
...
exported to "Output/Res-sex-pos-semtar.txt"
...
write.table(Output,"Output/Res-sex-pos-semtar.txt", sep = "\t", row.names = FALSE)
...
matrices, but adjusted for the matrix differences.
...
#' The rowdata of vamspos is put into the result matrix. Likewise the imputed data
#' is redefined into the container Data
res_matrix_pos_All <- rowData(SemiTargeted)
Data <- log2(assays(SemiTargeted)$Imputed)[, ]
...
on the individual matrices. Interestingly, Multiple samples with an adjusted 
p-values below 0.01 were found, but no feature with a fold change
above 2. This is in contrast to the individual models, where some features with
a large fold change, but no significant p-values were found.
...
#' Histograms for the pvalues of the combined matrices are made.
...
40, and a very steep decrease. This indicates that the effect of confounding 
values are likely to be minor.
...
16 features were found to have unadjusted p-values below 0.01. This is more than
double than any of the individual matrices by themselves.

...
abline(v = rowData(std_res)[ft, "mzmed"], lty = 3, col = "#ff000060", lwd = 2)
```
For Phosphorylethanolamine a peak can be seen in all three matrices.
...
side, whereas the venous samples shows peaks with very high intensity right
next to the rt Carnosine is expected at. 
...
```{r MS1-spectrum-Cystine, fig.path = IMAGE_PATH, fig.width = 10 * phi, fig.height = 6, fig.cap = "MS1 spectra of Cystine in a plasma sample (left) and in a capillary blood sample (right).", echo = FALSE, eval = TRUE}
par(mfrow = c(1, 2))
plotSpectra(sp_plas, labels = label_fun, labelPos = 4, labelOffset = 0.2,
            labelSrt = 30)
grid()
...
the capillary blood sample on the right side.
...
abline(v = rowData(std_res)[ft, "mzmed"], lty = 3, col = "#ff000060", lwd = 2)
```
On the left side, only a small peak at low intensity can be observed, whereas
in capillary blood (mid) and venous blood samples (right) peaks with higher
intensity were detected for dGMP.

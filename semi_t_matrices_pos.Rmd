...
for (std_current in std_matrices) {
    ...
    if (length(idx)) {
        ...
        names(vals) <- c("Capillary", "Plasma", "Venous")
        col <- col_source[tolower(names(vals))]
        png(paste0(FIG_PATH, rowData(std_res)$name[idx], "_matrices_POS.png"),
            width = 3.5, height = 4, units = "cm", res = 600,
            pointsize = 3)
        par(mar = c(4, 5, 3, 1))
        beeswarm(vals, bg = paste0(col, "cc"), col = "#00000040",
...
for (std_current in std_sex) {
    ...
    idx <- grep(std_current, rowData(std_res)$name)
    if (length(idx)) {
        vals <- split(tmp[idx, ], f = std_res$sex)
        names(vals) <- c("Female", "Male")
        col <- col_sex[c("F", "M")]
        png(paste0(FIG_PATH, rowData(std_res)$name[idx], "_sex_POS.png"),
            width = 3.5, height = 4, units = "cm", res = 600,
            pointsize = 3)
        par(mar = c(4, 5, 3, 1))
        beeswarm(vals, bg = paste0(col, "cc"), col = "#00000040",

...
such as *FG.001*, *FG.034* or *FG.175*. 

```{r pairwise-correlation-fg001, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.001.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- rownames(featureDefinitions(data_study))[
    grep("FG.001", featureGroups(data_study))]
pairs(t(fvals[fts, ]), gap = 0.1, main = "FG.0.01")
```

Correlation can be seen for some features (between FT0577 and FT0582 as well as
between FT0716, FT0909 and FT0204), but most are uncorrelated to each
other in this initial feature group.

```{r pairwise-correlation-fg034, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.034.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- rownames(featureDefinitions(data_study))[
    grep("FG.034", featureGroups(data_study))]
pairs(t(fvals[fts, ]), gap = 0.1, main = "FG.034")
```

Correlations can't be seen between some the features in this retention 
time feature group.

```{r pairwise-correlation-fg175, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.175.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- rownames(featureDefinitions(data_study))[
    grep("FG.175", featureGroups(data_study))]
pairs(t(fvals[fts, ]), gap = 0.1, main = "FG.175")
```

Other feature groups, such as *FG.082* were not splitted.

```{r pairwise-correlation-fg082, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.082.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- rownames(featureDefinitions(data_study))[
    grep("FG.082", featureGroups(data_study))]
pairs(t(fvals[fts, ]), gap = 0.1, main = "FG.082")
```
...
with *FG.007.001* which was split into 2 different feature groups based on EIC 
...
fts <- grep("FG.007.001", featureGroups(data_study))
...
```{r EICs-FG.007.001, fig.path = IMAGE_PATH, fig.cap = "EICs for all features from the inital feare gro FG.007.001. Subgrops were colored differenly.", echo = FALSE, fig.height = 10, fig.width = 10}
...
We evaluate next the sub-grouping in another feature group (*FG.085.001*) that
was split into 3 distinct groups.

```{r EIC-extraction-FG.085.001, echo = TRUE}
fts <- grep("FG.085.001", featureGroups(data_study))
eics <- featureChromatograms(data_study, features = fts,
                             filled = TRUE, n = 1)
```
...
```{r EICs-FG.085.001,  fig.path = IMAGE_PATH, fig.cap = "EICs for all features from the inital feare gro FG.085.001. Subgrops were colored differenly.", echo = FALSE, fig.height = 10, fig.width = 10}
cols <- brewer.pal(n = 9, "Set1")
fgs <- unique(featureGroups(data_study)[fts])
cols <- cols[seq_along(fgs)]
names(cols) <- fgs
...
Few feature group were not splitted but seems to contain signal from
different ions (signal shifted by ~ one scan). Increasing the required
correlation coefficient or a different similarity measure might work better.

```{r echo = FALSE}
## knitr::include_graphics(paste0(dr, c("FG.001.002.001.png",
##                                      "FG.006.002.001.png",
##                                      "FG.018.001.001.png",
##                                      "FG.039.001.001.png",
##                                      "FG.045.001.001.png",
...
Some feature groups consisting of many features (at least 4) seem to be
correctly grouped.

```{r echo = FALSE}

## knitr::include_graphics(paste0(dr, c("FG.041.001.001.png",
##                                      "FG.048.001.001.png",
##                                      "FG.053.001.001.png",
##                                      "FG.063.001.001.png",
##                                      "FG.067.001.001.png",
...
knitr::include_graphics(paste0(dr, c("FG.010.001.001.png",
                                     "FG.020.001.001.png",
                                     "FG.027.001.001.png",
                                     "FG.028.001.001.png",
                                     "FG.029.001.001.png",

...
We now want to group the significant features that derive from the same ion 
into one bin. This analysis is performed on a subset in three steps; we start 
by grouping the features according to their retention time, we then proceed by 
separating features within their groups according to their coefficients, which 
is an indicator for the abundance of each feature, and lastly, we refine the 
groups by similarity of their EIcs.
...
fgs[rownames(rowData(res))[rowData(res)$significant.CapvsVen]] <- "FG"
...
```{r plot-retention-time, fig.path = IMAGE_PATH, fig.width = 15, fig.height = 10, fig.cap = "Distribution of retention time and m/z of all features.", echo = FALSE}
...
on their (log2 transformed) abundances across samples with a heatmap.
...
res <- pheatmap(cormat, annotation_row = ann, cluster_rows = TRUE,
                cluster_cols = TRUE)
```
The feature groups are divided into smaller clusters according to their log2
transformed abundances.

We then refine our feature groups using the *groupFeatures* with the
*AbundanceSimilarityParam* to group features with a correlation higher than 0.5 
...
We evaluate this for one specific feature group "FG.0082" by plotting their 
pairwise correlation.

```{r pairwise-correlation, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.0082.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- grep("FG.034", featureGroups(data_study))
...
feature group. So, we first identify the 10 amples with highest intensity, then
...
with FG.011.001 which was split into two different feature groups based on EIC 
...
fts <- grep("FG.011.001", featureGroups(data_study))
...
```{r EICs-FG.011.001,  fig.path = IMAGE_PATH, fig.cap = "EICs for all features from the inital feare gro FG.011.001. Subgrops were colored differenly.", echo = FALSE, fig.height = 10, fig.width = 10}
cols <- c("#ff000080", "#00ff0080")
names(cols) <- unique(featureGroups(data_study)[fts])

par(mfrow = c(1, 2))
plotChromatogramsOverlay(eics, col = cols[featureGroups(data_study)[fts]],
                         lwd = 2, peakType = "none")
plotChromatogramsOverlay(normalize(eics),
                         col = cols[featureGroups(data_study)[fts]],
                         lwd = 2, peakType = "none")
```
...
```{r EIC-extraction-FG2, echo = TRUE}
fts <- grep("FG.080.001", featureGroups(data_study))
eics <- featureChromatograms(data_study, features = fts,
                             filled = TRUE, n = 1)
```
...
```{r EICs-FG.080.001,  fig.path = IMAGE_PATH, fig.cap = "EICs for all features from the inital feare gro FG.011.001. Subgrops were colored differenly.", echo = FALSE, fig.height = 10, fig.width = 10}
cols <- c("#ff000080", "#00ff0080")
names(cols) <- unique(featureGroups(data_study)[fts])

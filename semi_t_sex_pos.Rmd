...
<!--
**Modified**: `r file.info("semi_t_sex_pos.Rmd")$mtime`<br />
**Compiled**: `r date()`
-->
...
![](images/semi_t_sex_pos/standards-beeswarm-cap/FT1481_SAH.png)
In our data, the SAH levels in male samples are higher then in female samples.

![](images/semi_t_sex_pos/standards-beeswarm-cap/FT0312_Citrulline.png)
Also, the intensity of Citrulline and L-Aspartic acid peaks are higher in male 
compared to female samples.

![](images/semi_t_sex_pos/standards-beeswarm-cap/FT0168_L-Aspartic Acid.png)
...
![](images/semi_t_sex_pos/standards-beeswarm-cap/FT0168_L-Aspartic Acid.png)
![](images/semi_t_sex_pos/standards-beeswarm-ven/FT0527_C5 Carnitine.png)
...
![](images/semi_t_sex_pos/standards/SAH-FT1481.png)

Also the selected feature for Citrulline seems to be OK:

![](images/semi_t_sex_pos/standards/Citrulline-FT0312.png)
...
![](images/semi_t_sex_pos/standards/L-Aspartic Acid-FT0168.png)
...
![](images/semi_t_sex_pos/standards/C5 Carnitine-FT0527.png)
...
# Known sex-related compounds

In a next step, we looked for known sex-related compounds in the literature and
sub-set our list of lab-internal standards.
...
The sex-related compounds are summarized in this table:

```{r result-table-ft-std-selected, echo = FALSE, results = "asis"}
## Write result table
...
pandoc.table(sex_related_features[, c("name", "HMDB")],
             ...
             literature.")
```
Next, only the features assigned to these compounds are taken into consideration
and are subsetted in the `std_res_MF` object.
...
The subsetting reduced the number of features to `r length(std_res_MF)`. 

A PCA analysis is then performed on the subset to verify whether anything has
changed and if any similarities among the samples are visible or not.

```{r standards-pca-all-2, echo = FALSE}
pc <- prcomp(t(log2(assay(std_res_MF, "normalized_filled_imputed"))),
                 center = TRUE, scale. = FALSE)
```

```{r standards-pca-plot-2, fig.path = IMAGE_PATH, fig.cap = "PCA of the samples based on intensities of known sex-related compounds.", fig.width = 7 * phi, fig.height = 7, echo = FALSE}
par(mfrow = c(1, 2))
...
legend("topleft", col = col_source, legend = names(col_source),
       title = "phenotype", pch = 16, ncol = 2)
```
We then exclude the `RBC` samples from our analysis, as these samples showed
...
Subsequently, we re-perfom the PCA analysis:

```{r standards-pca-all-selected, echo = FALSE}
pc <- prcomp(t(log2(assay(std_res_MF, "normalized_filled_imputed"))),
                 center = TRUE, scale. = FALSE)
```

```{r standards-pca-plot-noRBC, fig.path = IMAGE_PATH, fig.cap = "PCA of the samples based on intensities of known sex-related compounds.", fig.width = 7 * phi, fig.height = 7, echo = FALSE}
...
legend("topleft", col = col_sex, legend = names(col_sex),
       title = "phenotype", pch = 16, ncol = 2)
```
We then subset according to the sample sources, to get a better resolution
of a possible gender effect.

```{r source-subset 2, echo = FALSE}
...
Then, we perform a PCA for the capillary blood samples:

```{r standards-pca-cap, echo = FALSE}
pc <- prcomp(t(log2(assay(res_cap, "normalized_filled_imputed"))),
                 center = TRUE, scale. = FALSE)
```

```{r standards-pca-plot-cap, fig.path = IMAGE_PATH, fig.cap = "PCA of the samples based on intensities of known sex-related compounds.", fig.width = 7 * phi, fig.height = 7, echo = FALSE}
...
legend("topleft", col = col_sex, legend = names(col_sex),
       title = "phenotype", pch = 16, ncol = 2)
```
Then, we perform the analysis on the venous blood samples:

```{r standards-pca-ven, echo = FALSE}
pc <- prcomp(t(log2(assay(res_ven, "normalized_filled_imputed"))),
                 center = TRUE, scale. = FALSE)
```

```{r standards-pca-plot-ven, fig.path = IMAGE_PATH, fig.cap = "PCA of the samples based on intensities of known sex-related compounds.", fig.width = 7 * phi, fig.height = 7, echo = FALSE}
...
And lastly, a PCA on plasma samples was performed:

```{r standards-pca-plas, echo = FALSE}
pc <- prcomp(t(log2(assay(res_plas, "normalized_filled_imputed"))),
                 center = TRUE, scale. = FALSE)
```

```{r standards-pca-plot-plas, fig.path = IMAGE_PATH, fig.cap = "PCA of the samples based on intensities of known sex-related compounds.", fig.width = 7 * phi, fig.height = 7, echo = FALSE}

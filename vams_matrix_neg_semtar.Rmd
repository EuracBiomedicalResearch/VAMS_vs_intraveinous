---
title: "Matrix comparison, semi-targeted analysis, negative polarity"
...
**Modified**: `r file.info("vams_matrix_neg_semtar.Rmd")$mtime`<br />
**Compiled**: `r date()`
...
#' Define paths:
filename <- "vams_matrix_neg_semtar"
...
[vams_matrix_neg.html](vams_matrix_neg.html)) we restrict the analysis to
...
vams_neg_semtar <- vams_neg[rownames(std_features_ann), ]
rowData(vams_neg_semtar) <- cbind(rowData(vams_neg_semtar), std_features_ann)
```

# *Qualitative* data analysis

Similar to the corresponding section in the analysis on the full data set
[vams_matrix_neg.html](vams_matrix_neg.html) we perform first a *qualitative*
...
[vams_matrix_neg.Rmd](vams_matrix_neg.Rmd), respectively the compiled html/pdf
...
prop_detected <- apply(assay(vams_neg_semtar, "weights"), 1, function(x, grps) {
  vapply(split(x, grps), function(z) sum(z %in% wght_detected) / length(z),
           numeric(1)) }, grps = vams_neg_semtar$source)
...
As before, most features are shared between all matrices, followed by all
matrices containing cell content (i.e. all except plasma). Only very few samples
are unique to a matrix. We next evaluate the average abundance of common or
...
ints <- list(
    common = rowMeans(assay(vams_neg_semtar)[feats_common, ]),
    plasma = rowMeans(assay(vams_neg_semtar)[feats_plasma,
                                             vams_neg_semtar$source == "plasma"]),
    venous = rowMeans(assay(vams_neg_semtar)[feats_venous,
                                         vams_neg_semtar$source == "venous"]),
    RBC = rowMeans(assay(vams_neg_semtar)[feats_RBC,
                                          vams_neg_semtar$source == "RBC"]),
    capillary = rowMeans(assay(vams_neg_semtar)[feats_capillary,
                                                vams_neg_semtar$source == "capillary"])
)
par(mar = c(7, 4.5, 1, 1))
...
tmp <- log2(assay(vams_neg_semtar, "imputed"))
pc <- prcomp(t(tmp), scale = FALSE, center = TRUE)
pchs <- ifelse(vams_neg_semtar$sex == "F", 21, 22)

par(mfrow = c(1, 2), mar = c(4.5, 4.5, 1, 1))
plot_pca(pc, pc_x = 1, pc_y = 2, 
         col = paste0(col_source[vams_neg_semtar$source], "ce"),
         bg = paste0(col_source[vams_neg_semtar$source], 60),
         pch = pchs, cex = 1.5)
...
set (see [vams_matrix_neg.html](vams_matrix_neg.html) and also
[vams_matrix_pos.html](vams_matrix_pos.html)).
...
fitMatrix <- t(apply(log2(assay(vams_neg_semtar, "imputed")), 1,
                     fitlmFunction, 
                     dataMat = as.data.frame(colData(vams_neg_semtar)),
...
res_matrix_neg <- cbind(rowData(vams_neg_semtar),
                        fitMatrix,
                        adj = padj,
                        significant = sign)
...
res_matrix_posl <- cbind(rowData(vams_neg_semtar),
                         pvalue = fit$p.value,
                         adj.pvalue = padj,
...
Most of the features were found to have statistically significant abundances
(compared to capillary samples) for plasma, RBC and venous samples,
respectively.
...
In contrast to the full data set (but similar to semi-targeted analysis of the positive polarity data), the distribution of m/z and retention times
for detected features of the measured standards are very similar between the
matrices. The only observable difference is in the m/z range around 300 where we
have an enrichment of features in capillary and RBC samples and a depletion in
plasma samples.
...
*vams_matrix_neg_semtar_results.xlsx* file.

```{r writexl}
library(writexl)
write_xlsx(as.data.frame(res_matrix_neg),
           path = "data/xls/vams_matrix_neg_semtar_results.xlsx")

save(res_matrix_neg, file = paste0(RDATA_PATH, "res_matrix_neg_semtar.RData"))
```

...
# Introduction
Having found the differences between each matrix in [vams_matrix_pos.Rmd](vams_matrix_pos.Rmd).
...
res_matrix_pos <- rowData(vams_pos)
Data <- log2(assays(vams_pos)$Imputed)[, ]

#A for loop in which each of the matrices are are passed to the above function
for (src in TestedSourcenames) {

fitMatrix <- t(apply(Data[, which(vams_pos$source == src)], 1, fitlmSexFunction, 
  dataMat = colData(vams_pos)[which(vams_pos$source == src), ], model = conc ~ sex))
...
plasma is where we would expect to see differences between genders.
...
res_matrix_pos_Conf <- rowData(vams_pos)
Data <- log2(assays(vams_pos)$Imputed)[, ]

#As before #A for loop in which each of the matrices are are passed to the above function
for (src in TestedSourcenames) {
fitMatrix <- t(apply(Data[, which(vams_pos$source == src)], 1, fitlmSexFunction, 
  dataMat = colData(vams_pos)[which(vams_pos$source == src), ], model = conc ~ sex + age))
...
The others are mostly the same whereas the capillary changes from about 250 to
about 630, a more than a factor 2 difference
...
#The rowdata of vamspos is put into the result matrix. Likewise the imputed data
#is redefined into the container Data
res_matrix_pos_All <- rowData(vams_pos)
Data <- log2(assays(vams_pos)$Imputed)[, ]

#A for loop in which each of the matrices are are passed to the above function

fitMatrix <- t(apply(Data[, ], 1, fitlmSexFunction, 
  dataMat = colData(vams_pos)[, ], model = conc ~ sex + source))
...
1500, and a very steep decrease. This indicates that the effect of confounding 
...
661 features were found to have unadjusted p-values below 0.01. This is more 

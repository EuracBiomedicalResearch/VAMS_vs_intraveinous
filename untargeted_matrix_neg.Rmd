...
png(paste0(FIG_PATH, "FIG_NEG_PCA_untargeted.png"), width = 9, height = 4.5,
...
Correlation can be seen for some features (between FT00626 and FT05508), but 
most are uncorrelated to each other in this initial feature group.

```{r pairwise-correlation-fg114, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.034.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- rownames(featureDefinitions(data_study))[
    grep("FG.114", featureGroups(data_study))]
pairs(t(fvals[fts, ]), gap = 0.1, main = "FG.114")
```

Correlation can be seen for some features (between FT11396 and FT13447 as well
as between FT1447 and FT13635), but most are uncorrelated to each
other in this initial feature group.

Other feature groups, such as *FG.210* were not splitted.

```{r pairwise-correlation-fg236, fig.path = IMAGE_PATH, fig.cap = "Pairwise correlation of features initially grouped in feature group FG.082.", echo = FALSE, fig.height = 10, fig.width = 10}
fts <- rownames(featureDefinitions(data_study))[
...
```{r EICs-FG.171.001, fig.path = IMAGE_PATH, fig.cap = "EICs for all features from the inital feare gro FG.007.001. Subgrops were colored differenly.", echo = FALSE, fig.height = 10, fig.width = 10}
...
```{r EICs-FG.216.001,  fig.path = IMAGE_PATH, fig.cap = "EICs for all features from the inital feare gro FG.216.001. Subgrops were colored differenly.", echo = FALSE, fig.height = 10, fig.width = 10}
cols <- brewer.pal(n = 9, "Set1")
fgs <- unique(featureGroups(data_study)[fts])
cols <- cols[seq_along(fgs)]
names(cols) <- fgs
...
Few feature groups were not splitted but seem to contain signal from

...
fitting is skipped for features with less than 12 valid signals.
...
residual larger than some value (which flagged models that had noisy signal in
QC samples).
...
filled-in values) based on the estimated models. Adjustment resulted in 270
...
Within-batch adjustment did **increase** the 75% MRAs slightly, especially its
75% quantile and the value ranges. Also the percentage of features of replicated
samples with more than 2-fold differences is increase. This could partially be
explained if QC sample measurements are not representative for all matrices.

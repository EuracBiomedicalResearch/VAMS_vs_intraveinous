...
Below we fit linear models `y ~ inj_idx + source` to the log2 transformed 
abundances (only of detected peaks) using robust regression [@Koller:2017jsa]. 
This model estimates the injection order dependent signal drift on all
samples, accounting however for the different types of source (i.e. allow a
different average abundance for samples of the same matrix). Model fitting is 
skipped for features with less than 12 valid signals.
...
mdls_a <- xcms:::rowFitModel(
                     y ~ inj_idx + source, data = pData(data_neg)[smpls_a, ],
...
mdls_b <- xcms:::rowFitModel(
                     y ~ inj_idx + source, data = pData(data_neg)[smpls_b, ],
...
for (ft in fts) {
    ...
    lmoda <- xcms:::fitModel(y ~ inj_idx, data = pData(data_neg)[qcs_a, ],
                             method = "lmrob", y = y_sub[qcs_a])
    lmodb <- xcms:::fitModel(y ~ inj_idx, data = pData(data_neg)[qcs_b, ],

...
which is thus used below.As this median scaling approach tend to be biased by 
the low intensities of metabolites calculated in the gap-filling process, only 
the top 1000 features with highest abundances per sample were used for median 
scaling.
...
The normalization factores are calculated on the top x features with highest
abundance per sample. The normalization will hence be independent of gap-filling.
We assume that the intensities of the top x highest intensity features is 
comparable across samples (independently which compound/metabolite these 
features might be).
...
```{r PCA-raw-normalized, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances. Circles and rectangles indicate sample different batches.", echo = FALSE}
...
```{r PCA-raw-normalized POOL, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances. Circles and rectangles indicate sample different batches.", echo = FALSE}
## Perform data imputation for PCA
...
```{r PCA-raw-normalized plasma, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances. Circles and rectangles indicate sample different batches.", echo = FALSE}
## Perform data imputation for PCA
...
```{r PCA-raw-normalized venous, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances. Circles and rectangles indicate sample different batches.", echo = FALSE}
## Perform data imputation for PCA
...
```{r PCA-raw-normalized capillary, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances. Circles and rectangles indicate sample different batches.", echo = FALSE}
## Perform data imputation for PCA
...
```{r PCA-raw-normalized RBC, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances. Circles and rectangles indicate sample different batches.", echo = FALSE}
## Perform data imputation for PCA
...
```{r PCA-raw-normalized-IS, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of raw and normalized feature abundances for internal standards. Circles and rectangles indicate sample different batches.", echo = FALSE}
pc_raw <- prcomp(
...
tmp <- assay(res_neg, "raw_filled")
tmp[is.na(assay(res_neg, "raw"))] <- NA
rsd_raw_det <- rowRsd(tmp[, res_neg$sample == "POOL"])
tmp <- assay(res_neg, "median_filled")
tmp[is.na(assay(res_neg, "raw"))] <- NA
rsd_scl_det <- rowRsd(tmp[, res_neg$sample == "POOL"])
tmp <- assay(res_neg, "within_filled")
tmp[is.na(assay(res_neg, "raw"))] <- NA
rsd_adj_det <- rowRsd(tmp[, res_neg$sample == "POOL"])
tmp <- assay(res_neg, "normalized_filled")
tmp[is.na(assay(res_neg, "raw"))] <- NA
rsd_bbn_det <- rowRsd(tmp[, res_neg$sample == "POOL"])
```
...
tmp_is <- assay(res_neg, "raw_filled")[is_std$feature_id, ]
rsd_raw_is <- rowRsd(tmp_is[, res_neg$sample == "POOL"])
tmp_is <- assay(res_neg, "median_filled")[is_std$feature_id, ]
rsd_scl_is <- rowRsd(tmp_is[, res_neg$sample == "POOL"])
tmp_is <- assay(res_neg, "within_filled")[is_std$feature_id, ]
rsd_adj_is <- rowRsd(tmp_is[, res_neg$sample == "POOL"])
tmp_is <- assay(res_neg, "normalized_filled")[is_std$feature_id, ]
rsd_bbn_is <- rowRsd(tmp_is[, res_neg$sample == "POOL"])

## Calculate RSD on good internal standards in study samples
tmp_is <- assay(res_neg, "raw_filled")[is_std$feature_id, ]
rsd_raw_is_sts <- rowRsd(tmp_is[, res_neg$sample != "POOL"])
tmp_is <- assay(res_neg, "median_filled")[is_std$feature_id, ]
rsd_scl_is_sts <- rowRsd(tmp_is[, res_neg$sample != "POOL"])
tmp_is <- assay(res_neg, "within_filled")[is_std$feature_id, ]
rsd_adj_is_sts <- rowRsd(tmp_is[, res_neg$sample != "POOL"])
tmp_is <- assay(res_neg, "normalized_filled")[is_std$feature_id, ]
rsd_bbn_is_sts <- rowRsd(tmp_is[, res_neg$sample != "POOL"])
...
boxplot(list(raw = rsd_raw_det, scaled = rsd_scl_det, within = rsd_adj_det,
             normalized = rsd_bbn_det),
...
T <- rbind(`mean RSD` = c(`raw - QC samples` = mean(rsd_raw, na.rm = TRUE),
                          `scaled - QC samples` = mean(rsd_scl, na.rm = TRUE),
                          `within-batch - QC samples` = mean(rsd_adj, na.rm = TRUE),
                          `between-batch - QC samples` = mean(rsd_bbn, na.rm = TRUE),
                          `raw detected - QC samples` = mean(rsd_raw_det, na.rm = TRUE),
                          `scaled detected - QC samples` = mean(rsd_scl_det, na.rm = TRUE),
                          `within-batch detected - QC samples` = mean(rsd_adj_det,
                                                       na.rm = TRUE),
                          `between-batch detected - QC samples` = mean(rsd_bbn_det,
                                                       na.rm = TRUE),
                          `raw - Study samples` = mean(rsd_raw_sts,
                                                       na.rm = TRUE),
                          `scaled - Study samples` = mean(rsd_scl_sts,
                                                          na.rm = TRUE),
                          `within-batch - Study samples` = mean(rsd_adj_sts,
                                                              na.rm = TRUE),
                          `between-batch - Study samples` = mean(rsd_bbn_sts,
                                                              na.rm = TRUE),
                          `raw IS` = mean(rsd_raw_is, na.rm = TRUE),
                          `scaled IS` = mean(rsd_scl_is, na.rm = TRUE),
                          `within-batch IS` = mean(rsd_adj_is, na.rm = TRUE),
                          `between-batch IS` = mean(rsd_bbn_is, na.rm = TRUE),
                          `raw IS - Study samples` = mean(rsd_raw_is_sts,
                                                          na.rm = TRUE),
                          `scaled IS - Study samples` = mean(rsd_scl_is_sts,
                                                             na.rm = TRUE),
                          `within-batch IS - Study samples` = mean(rsd_adj_is_sts,
                                                                 na.rm = TRUE),
                          `between-batch IS - Study samples` = mean(rsd_bbn_is_sts,
                                                                 na.rm = TRUE)),
           ...
           `% RSD > 0.3` = c(c(sum(rsd_raw > 0.3, na.rm = TRUE),
                             ...
                             sum(rsd_bbn_det > 0.3, na.rm = TRUE)) *
...
cpt <- paste0("Summary of RSD calculations. We show how the RSD changes among ",
              ...
              "study samples.")
pandoc.table(t(T), caption = cpt, style = "rmarkdown")
```
...
tab <- rbind(
    ...
    median = colMeans(repl_med[, c("R", "90% log2FC")]),
    within = colMeans(repl_within[, c("R", "90% log2FC")]),
    normalized = colMeans(repl_between[, c("R", "90% log2FC")])
)

...
tmp_perc <- apply(fsumm[, paste0(study_src, "_perc")], 1,
                  function (z) any(z > 30))
fts_used <- rownames(
    featureDefinitions(data_neg))[which(tmp < 0.1 & tmp_perc)]
...
fcp <- FillChromPeaksParam(
    ppm = 20, fixedRt = median(chromPeaks(data_neg)[, "rtmax"] -
                               chromPeaks(data_neg)[, "rtmin"]) / 2)
...
##' First all features.
tmp_pks <- chromPeaks(data_neg)
dpks_dens <- density(log2(tmp_pks[!chromPeakData(data_neg)$is_filled, "into"]))
fpks_dens <- density(log2(tmp_pks[chromPeakData(data_neg)$is_filled, "into"]))
...
tmp_pks <- chromPeaks(data_neg)[pk_idxs, ]
dpks_dens <- density(
    log2(tmp_pks[!chromPeakData(data_neg)$is_filled[pk_idxs], "into"]))
fpks_dens <- density(
    log2(tmp_pks[chromPeakData(data_neg)$is_filled[pk_idxs], "into"]))
yl <- c(0, max(dpks_dens$y, fpks_dens$y))
xl <- range(dpks_dens$x, fpks_dens$x)
...
fv <- featureValues(data_neg, method = "sum", value = "into")[fts_used, ]
fv_raw <- fv
fv_nofill <- featureValues(data_neg, method = "sum",
...
cols <- col_source[data_neg$source]
par(mar = c(0.2, 4.5, 0.2, 0.5))
barplot(apply(fv_nofill, MARGIN = 2, function(x) sum(!is.na(x))),
        col = paste0(cols, 80), ylab = "features", xaxt = "n")
dobox(log2(fv_nofill), col = paste0(cols, "ff"), xaxt = "n",
...
Some of the RBC (red) samples also have a considerably lower number of detected
features. The distribution of feature intensities seems to be comparable between
samples, sources and batches. The high similarity of average abundances between
samples visible above might however be misleading, since only signal from
identified peaks are considered. Comparing signal intensities **after**
filling-in missing peak data might be better, because it also reflects the fact
that some features are simply not present in the data.
...
barplot(apply(fv, MARGIN = 2, function(x) sum(!is.na(x))),
        col = paste0(cols, 80), ylab = "features", xaxt = "n")
dobox(log2(fv), col = paste0(cols, "ff"), xaxt = "n", border = paste0(cols, 60))
...
visible (RBC with a lower percentage of detected peaks have a lower lower
quantile of peak intensities).
...
cols <- col_source[data_neg$source]
boxplot(rowRla(fv_nofill, group = data_neg$source), cex = 0.5, pch = 16, 
        col = cols, ylab = "RLA", border = paste0(cols, 40), notch = TRUE,
        outline = FALSE, xaxt = "n", main = "raw data")
grid(nx = NA, ny = NULL)
abline(v = sum(data_neg$batch == 6022018) + 0.5, col = "grey")

```

The RLA plot above shows some potentially systematic, wave-like, drifts in
signal intensities. Next we plot the RLA plots after filling-in missing peak
data.
...
cols <- col_source[data_neg$source]
boxplot(rowRla(fv, group = data_neg$source), cex = 0.5, pch = 16, 
        col = cols, ylab = "RLA", border = paste0(cols, 40), notch = TRUE,
...
After filling-in missing peak data differences for RBC samples become more
pronounced.
...
crlas <- colMeans(rowRla(fv, group = data_neg$source), na.rm = TRUE)
injidx <- data_neg$inj_idx
injidx[data_neg$batch == 7022018] <- injidx[data_neg$batch == 7022018] + 100
...
There seems to no common systematic trend.
...
fts_int_std <- lapply(int_std$mz, function(mz) {
    featureDefinitions(data_neg, mz = mz, ppm = 10, type = "apex_within")
})
...
#' col <- c("#ce000080", "#0000ce80")[as.integer(data_neg$batch)]
...
for (i in 1:length(fts_int_std)) {
    ...
    fn <- gsub("%", "", paste0(dr, "int_std_", i, "_",
                               int_std[i, "name"], ".png"))
    fts <- fts_int_std[[i]]
    chrs <- featureChromatograms(data_neg, features = rownames(fts),
                                 expandRt = 5)
...
#' Aspartic acid: 183.6
#' L-Cystine: 210
#' L-Glutamic acid: 175.2
#' L-Histidine: 190.2
#' L-Isoleucine: 148.2
#' L-Leucine: 144.6
#' L-Lysine: 190.2
#' L-Methionine: 157.2
#' L-Phenylalanine: 154.2
#' L-Proline: 168.6
#' L-Serine: 182.4
#' L-Threonine: 174.6
#' L-Tyrosine: 168
...
##' pks <- chromPeaks(data_neg)[fts$peakidx[[1]], ]
##' rect(xleft = pks[, "rtmin"], xright = pks[, "rtmax"], ybottom = 0,
##'      ytop = pks[, "maxo"], border = "#ff000080")
##' pks <- chromPeaks(data_neg)[fts$peakidx[[2]], ]
##' rect(xleft = pks[, "rtmin"], xright = pks[, "rtmax"], ybottom = 0,
##'      ytop = pks[, "maxo"], border = "#0000ff80")

##' Manually select features/internal standards.
int_std$feature <- rep(NA_character_, nrow(int_std))
int_std[6, "feature"] <- rownames(fts_int_std[[6]])[1]
#' int_std[7, "feature"] <- rownames(fts_int_std[[7]])    #' just huge differences
int_std[8, "feature"] <- rownames(fts_int_std[[8]]) #' very high in plasma
int_std[9, "feature"] <- rownames(fts_int_std[[9]]) #' OK
int_std[11, "feature"] <- rownames(fts_int_std[[11]]) #' OKish
int_std[13, "feature"] <- rownames(fts_int_std[[13]])[1]
int_std[14, "feature"] <- rownames(fts_int_std[[14]]) #' OK
#' int_std[18, "feature"] <- rownames(fts_int_std[[18]]) #' just huge differences
...
unambiguous. Note however that for some internal standards the peaks show large,
matrix-specific, differences which suggests their use in the data normalization
not to be ideal.

The table below lists the internal standards and their assigned
feature with mean abundance and its standard deviation.
...
tmp_grp_avg <- t(apply(tmp_fv, 1, function(z) {
    unlist(lapply(split(log2(z), data_neg$source), mean, na.rm = TRUE))
}))
...
col_2 <- col
## col_2 <- c("#ff000080", "#0000ff80")[as.numeric(data_neg$batch)]

fv_imp <- imputeRowMinRand(fv, min_fraction = 1/2, sd_fraction = 1)
...
possibly amount. We use the same approaches as described in
*vams_normalization*.
...
fv_imp <- imputeRowMinRand(fv, min_fraction = 0.5, sd_fraction = 1)
...
ruv_3 <- RUVIII(t(log2(fv_imp)), ctl = neg_ctrls_int, k = 6,
                M = design.matrix(data_neg$sample))
fv_ruv3 <- 2^t(ruv_3)
...
We calculate below the MRA (maximum ratio of abundances) between replicated
measurements (using only the detected signal). From these we calculate the 75%
quantile of the absolute MRA per sample as a proxy for between replicate
...
abundance, being the median scaling method and the MRM. Methods using the
internal standards for normalization (RUV and NOMIS) result in larger
...
dobox(log2(fv_raw), main = "raw data", ylim = c(5, 20), col = cols,
      border = paste0(cols, 60))
points(colMeans(log2(fv_raw), na.rm = TRUE), type = "l")
dobox(log2(fv_sm), main = "sum normalized", ylim = c(5, 20), col = cols,
      border = paste0(cols, 60))
points(colMeans(log2(fv_sm), na.rm = TRUE), type = "l")
dobox(log2(fv_mdn), main = "median normalized", ylim = c(5, 20), col = cols,
      border = paste0(cols, 60))
points(colMeans(log2(fv_mdn), na.rm = TRUE), type = "l")
dobox(log2(fv_mrm), main = "MRM normalized", ylim = c(5, 20), col = cols,
      border = paste0(cols, 60))
points(colMeans(log2(fv_mrm), na.rm = TRUE), type = "l")
dobox(log2(fv_ruvr), main = "RUV rand normalized", col = cols,
      border = paste0(cols, 60))
points(colMeans(log2(fv_ruvr), na.rm = TRUE), type = "l")
dobox(log2(fv_ruv3), main = "RUVIII normalized", col = cols,
      border = paste0(cols, 60))
points(colMeans(log2(fv_ruv3), na.rm = TRUE), type = "l")
dobox(log2(fv_nms), main = "NOMIS normalized", col = cols,
      border = paste0(cols, 60))
...
#' y ~ inj_idx
smps <- data_neg$batch == 6022018
lm_btch1 <- xcms:::rowFitModel(
                       y ~ inj_idx, y = log2(fv_nofill[, smps]),
                       method = "lmrob", minVals = 12,
                       data = data.frame(inj_idx = data_neg$inj_idx[smps]))
smps <- data_neg$batch == 7022018
...
dependent signal drift to abundances of all samples **separately** for the two
...
also in the other batch. 

Signal drift estimation based on measurements in QC samples resulted in
larger differences between replicated measurements (potentially because QC
samples might not be representative for all matrices).

Below we fit linear models `y ~ inj_idx` to the log2 transformed abundances 
(only of detected peaks) using robust regression [@Koller:2017jsa]. This model
estimates the injection order dependent signal drift on **all** samples. Model 
fitting is skipped for features with less than 12 valid signals. We fit the
linear models to the full data instead of only the QC samples as they do not
seem to be representative of all matrices.

```{r fit-model-batch, message = FALSE, warning = FALSE}
smpls_a <- data_neg$batch == "6022018"
qcs_a <- data_neg$sample == "POOL" & smpls_a
mdls_a <- xcms:::rowFitModel(
                     y ~ inj_idx, data = pData(data_neg)[smpls_a, ],
                     y = log2(fv_nofill[, smpls_a]), method = "lmrob",
                     minVals = 12)

smpls_b <- data_neg$batch == "7022018"
qcs_b <- data_neg$sample == "POOL" & smpls_b
mdls_b <- xcms:::rowFitModel(
                     y ~ inj_idx, data = pData(data_neg)[smpls_b, ],
                     y = log2(fv_nofill[, smpls_b]), method = "lmrob",
                     minVals = 12)

```

We next remove fitted linear models for features for which valid measurements do
not span at least 3/4 of the injection index range. This avoids extrapolation of
the signal drift. In contrast to a previous analysis (in which the signal drift
was estimated purely on QC samples) we do not flag models with a mean absolute
residual larger than some cut-off value (which flagged models that had noisy signal in
QC samples). Because we fit the linear models to the full data consisting of
signals from different matrices we do expect larger residuals.

```{r fit-model-table-batch, message = FALSE, echo = FALSE, fig.cap = "Distribution of slopes of the fitted models for batch a and b. Blue represents slopes for models flagged because of too low span of injection index.", fig.width = 8, fig.height = 4, fig.path = IMAGE_PATH}
...
flgs_inj_range_a <- vapply(mdls_a, flag_model_inj_range, logical(1),
                           min_range = diff(range(data_neg$inj_idx)) * 3/4)
...
bias. Note that the *injection index range filter* removed most of the linear
models with the largest estimated slopes/effects.
...
For most of the features a model describing the injection dependent signal drift
was defined.

Most of the slopes from the models describing the injection dependent signal
drift are close to 0 suggesting only a relatively low influence. Only 57 have a
absolute slope larger than 0.018.
...
dir.create(dr, showWarnings = FALSE)
fts <- names(mdls_b)[which(abs(slps_b) > 0.018)]
for (ft in fts) {
    png(paste0(dr, ft, ".png"), width = 10, height = 5, pointsize = 6,
        res = 200, units = "cm")
    plot_feature_slopes_batch(y = log2(fv[ft, ]),
...
#' Mostly those with values in a single source
```
...
Examples for linear models that were **excluded** because of QC sample measuring
a too small range of injection indices are shown below.
...
filled-in values) based on the estimated models. Adjustment resulted in 284
measurements with negative (log2) abundances. These were replaced by half of the
minimum non negative intensity for that feature.
...
#' Select some with the best R squared in batch a, one for batch b
...
fts <- unique(c("FT02558", "FT00387", fts))
...
for (ft in fts) {
    ...
    lmoda <- xcms:::fitModel(y ~ inj_idx, data = pData(data_neg)[smpls_a, ],
                             method = "lmrob", y = y_sub[smpls_a])
    lmodb <- xcms:::fitModel(y ~ inj_idx, data = pData(data_neg)[smpls_b, ],
                             method = "lmrob", y = y_sub[smpls_b])
    plot_feature_slopes_batch(y = log2(fv_adj[ft, ]),
                              is_filled = is.na(fv_nofill[ft, ]),
                              data = pData(data_neg), lmoda = lmoda,
...
```{r within-batch-before-example2, out.width = "750px", echo = FALSE, fig.cap = "Feature abundances before adjustment."}
knitr::include_graphics(paste0(dr, "FT02558_raw.png"))
```

```{r within-batch-after-example2, out.width = "750px", echo = FALSE, fig.cap = "Feature abundances after within-batch adjustment."}
knitr::include_graphics(paste0(dr, "FT02558_adj.png"))
```
...
The 75% MRA of replicated samples is between 1.2 and 1.8 which is quite
OK, meaning that most samples have a less than 2-fold difference between
...
Within-batch adjustment reduced the 75% MRAs slightly. Also the percentage of
features of replicated samples with more than 2-fold differences is lowered.

```{r featureValues-within-norm}
fv_adj <- fv_adj #' Replace with fv to skip within-batch normalization
```
...
this signal to QC ratio to be comparable between the two batches. For a
considerable number of features this differs however between the batches. In
such cases a feature-wise normalization based on QC samples would however
introduce a bias.
...
tmp <- fv_adj
tmp[is.na(fv_nofill)] <- NA
data_neg$batch <- factor(data_neg$batch) #' avoid batch being treated as number
mdls_batch <- xcms:::rowFitModel(y ~ batch,
                                 data = pData(data_neg)[qcs_a | qcs_b, ],
...
src_btch <- paste(data_neg$source, data_neg$batch)
...
btch_a <- grep("6022018", colnames(src_btch_avg))
src_btch_avg[, btch_a] <- src_btch_avg[, btch_a] -
    src_btch_avg[, "all 6022018"]
btch_b <- grep("7022018", colnames(src_btch_avg))
src_btch_avg[, btch_b] <- src_btch_avg[, btch_b] -
    src_btch_avg[, "all 7022018"]
...
plot_ft_int <- function(x, ft, main = ft, ...) {
    ...
    plot(log2(x[ft, ]), main = main, col = col_source[data_neg$source],
         pch = pchs)
    abline(v = sum(data_neg$batch == data_neg$batch[1]))
}
...
An example for a feature which will be normalized with a global model because
the difference between the average QC signal to average sample ratio between the
two batches is large is shown below.
...
mdls_batch[is.na(mdls_batch)] <- list(glbl_mdl_batch) #' global norm for some
...
dobox(log2(fv_raw), main = "raw data", only_detected = TRUE,
      col = paste0(cols, "ff"), border = paste0(cols, 60))
...
par(mfrow = c(2, 1), mar = c(1, 4.5, 1, 0.5))
dobox(rowRla(fv_raw, data_neg$source), main = "raw data",
      ylab = "RLA", only_detected = TRUE, col = paste0(cols, "ff"),
      border = paste0(cols, 60))
dobox(rowRla(fv, data_neg$source), main = "normalized data",
      ylab = "RLA", only_detected = TRUE, col = paste0(cols, "ff"),
      border = paste0(cols, 60))
...
the normalization factors are also estimated on QC samples this measure might be
biased.

```{r final-qa-rsd, message = FALSE, warning = FALSE}
rsd_raw <- rowRsd(fv_raw[, data_neg$source == "all"])
rsd_adj <- rowRsd(fv[, data_neg$source == "all"])

#' Only on detected signal
tmp <- fv_raw
tmp[is.na(fv_nofill)] <- NA
rsd_raw_det <- rowRsd(tmp[, data_neg$source == "all"])
tmp <- fv
tmp[is.na(fv_nofill)] <- NA
rsd_adj_det <- rowRsd(tmp[, data_neg$source == "all"])
...
peaks. Note that, since replicates were measured **within** each batch, this
will only allow to evaluate the **within** batch normalization.
...
The 75% MRA of replicated samples is between 1.2 and 1.8 which is quite OK,
...
Summarizing, normalization could improve data quality slightly.
...
grp <- paste0(data_neg$source, "-", data_neg$sample)
pd <- cbind(pData(data_neg), sample_name = grp,
...
#' Create feature data
fd <- featureDefinitions(data_neg)
fd <- cbind(fd[rownames(fv), colnames(fd) != "peakidx"],
            featureSummary(data_neg, data_neg$source)[rownames(fv), ],
            QC_rsd = rowRsd(fv[, data_neg$source == "all"]))
...
#'                  featureDefinitions(data_neg)[fts_used,
#'                                               c("mzmed", "mzmin", "mzmax",
#'                                                 "rtmed", "rtmin", "rtmax")],
...
#' fv <- featureValues(data_neg, method = "sum",
#'                     value = "into", filled = FALSE)[fts_used, ]

...
grid(nx = NA, ny = NULL)
abline(v = sum(data_pos$batch == 6022018) + 0.5, col = "grey")
...
(only of detected peaks) using robust regression [@Koller:2017jsa]. Model 
...
drift are close to 0 suggesting only a relatively low influence. Only 48 have a
absolute slope larger than 0.025.
...
for (ft in fts_a) {
    ...
    plot_feature_slopes_batch(y = log2(fv[ft, ]),
                              is_filled = is.na(fv_nofill[ft, ]),
                              data = pData(data_neg), lmoda = mdls_res_a[[ft]],

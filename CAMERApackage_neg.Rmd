...
combinedAdducts <- combinexsAnnos(anAdduct_pos, anAdduct_neg, pos = FALSE, tol=2, ruleset=NULL)
...
#' corresponding mz and retention times.
 ...
 rownames(ExportTable) <- rownames(featureDefinitions(data_neg))
...
save(ExportTable, file = paste0(RDATA_PATH, "ExportTableNeg.RData"))
write.table(as.matrix(ExportTable), 
  file = paste0(RDATA_PATH, "CAMERA_Annotations_neg.txt"),sep = "\t", 
  row.names = FALSE)

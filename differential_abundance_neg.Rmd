...
We then plot the heatmap with features with lowest p-values:

```{r standards-heatmap, fig.path = IMAGE_PATH, fig.width = 15, fig.height = 5, fig.cap = "Heatmap of features.", echo = FALSE, eval = nrow(tab) > 1}
tmp <- log2(assay(res_cap, "normalized_filled_imputed")[rownames(tab), , drop = FALSE])
tmp <- tmp - rowMeans(tmp, na.rm = TRUE)
ann <- as.data.frame(colData(res_cap)[, c("sex")])
pm <- pheatmap(tmp, annotation_col = ann, labels_col = 1:ncol(res_cap),
               annotation_color = list(sex = col_sex))
tmp <- data.frame(

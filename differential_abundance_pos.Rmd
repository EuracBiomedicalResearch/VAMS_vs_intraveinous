...
This data frame is then added to the `rowData` of the `std_res` object.

```{r standards-analysis, echo = FALSE}
source <- factor(res_pos$source)
sex <- factor(res_pos$sex)
...
contr_mat <- makeContrasts(
  VenvsCap = sourcevenous - sourcecapillary,
  levels = dsgn)
fit <- contrasts.fit(fit, contrasts = contr_mat)
fit <- eBayes(fit)

tmp <- data.frame(
    coef = fit$coefficient[, "VenvsCap"],
    pvalue = fit$p.value[, "VenvsCap"],
    adjp = p.adjust(fit$p.value[, "VenvsCap"]),
    avg.ARVD = rowMeans(
        log2(assay(
             std_res, "normalized_filled_imputed")[, std_res$source == "capillary"])),
    avg.CTRL = rowMeans(
        log2(assay(
             std_res, "normalized_filled_imputed")[, std_res$source == "venous"]))
    )

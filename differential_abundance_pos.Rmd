...
In total `r sum(rowData(res_cap)$significant)` feature we do not find a
...
**SAH** shows the smallest p-value, which is already described as being different in
concentration when male and female are compared. The second identifies metbolite
with a small p-value  is **Citrulline**: this is an amino acid that is produced 
from arginine during the process of nitrogen monoxide (NO) synthesis.
This biological messenger plays an important role in mechanical and chemical
signal transduction, especially in cardiomyocytes.**L-Aspartic acid** is an amino 
acid, which plays a major role in the production in antibodies. As the immune 
response is stronger in female compared to male, also the higher abundance of
this AA may be expected.
...
**Carnitine**, former also being found significant between male and female samples in
the capillary blood samples.
...
**Homocysteine** shows the lowest p-value here.
